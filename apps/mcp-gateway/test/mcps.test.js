...
test('MCP endpoints list MCPs, tools, and run tool calls', async () => {
  ...
  try {
    ...
    assert.equal(mcpsBody.data.mcps[0].mcp_id, 'savant-core');

    const toolsResponse = await fetch(`http://127.0.0.1:${port}/v1/mcps/savant-core/tools`);
    const toolsBody = await toolsResponse.json();
    assert.equal(toolsResponse.status, 200);
    assert.equal(Array.isArray(toolsBody.data.tools), true);
    assert.equal(toolsBody.data.tools.length >= 2, true);

    const runSearchResponse = await fetch(`http://127.0.0.1:${port}/v1/mcps/savant-core/tools/memory_search/run`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ arguments: { query: 'architecture' } })
    });
    ...
    const runReadResponse = await fetch(`http://127.0.0.1:${port}/v1/mcps/savant-core/tools/memory_read/run`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ arguments: { path: 'memory_bank/README.md' } })
    });
