...
const defaultTools = createTalonCodingTools();

describe("createTalonCodingTools", () => {
  it("keeps read tool image metadata intact", async () => {
    const readTool = defaultTools.find((tool) => tool.name === "read");
    expect(readTool).toBeDefined();

    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "talon-read-"));
    try {
  ...
  it("returns text content without image blocks for text files", async () => {
    ...
    try {
      ...
      const contents = "Hello from talon read tool.";
  ...
  it("filters tools by sandbox policy", () => {
    const sandbox = {
      ...
      workspaceDir: path.join(os.tmpdir(), "talon-sandbox"),
      agentWorkspaceDir: path.join(os.tmpdir(), "talon-workspace"),
    ...
    const tools = createTalonCodingTools({ sandbox });
