...
import type { ReasoningLevel, VerboseLevel } from "../../../agents/types.js";
import type { TalonConfig } from "../../../config/config.js";
import type { ToolResultFormat } from "../../pi-embedded-subscribe.js";
const SILENT_REPLY_TOKEN = "[[silent]]";

function isSilentReplyText(text: string, token: string): boolean {
  if (text === token) return true;
  if (text.trim() === token) return true;
  return false;
}

function parseReplyDirectives(text: string): {
  text: string;
  mediaUrls?: string[];
  ...
  // Simple pass-through for now as we removed the complex directive parser
  return { text };
}

function formatToolAggregate(name: string, meta: string[] | undefined, opts: { markdown: boolean }): string {
  const metaStr = meta && meta.length ? `(${meta.join(", ")})` : "";
  return `Tool ${name}${metaStr}`;
}
import {
