...
export function resolveAnnounceTargetFromKey(sessionKey: string): AnnounceTarget | null {
  ...
  const normalizedChannel = normalizeMessageChannel(channelRaw);
  const channel = normalizedChannel ?? channelRaw.toLowerCase();
  const kindTarget = kind === "channel" ? `channel:${id}` : `group:${id}`;
...
export function resolvePingPongTurns(cfg?: TalonConfig) {
