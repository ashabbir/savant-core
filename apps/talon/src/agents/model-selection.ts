...
export function normalizeProviderId(provider: string): string {
  ...
  if (normalized === "google" || normalized === "google-gemini-cli" || normalized === "google-api") {
...
export function isCliProvider(provider: string, cfg?: TalonConfig): boolean {
...
export function resolveConfiguredModelRef(params: {
  ...
  if (rawModel) {
    ...
    if (!trimmed.includes("/")) {
      ...
      console.warn(
        `[talon] Model "${trimmed}" specified without provider. Falling back to "anthropic/${trimmed}". Please use "anthropic/${trimmed}" in your config.`,
      );
