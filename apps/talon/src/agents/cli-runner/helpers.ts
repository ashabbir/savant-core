...
/**
 * Cleanup suspended Talon CLI processes that have accumulated.
...
function buildModelAliasLines(cfg?: TalonConfig) {
...
export async function writeCliImages(
  ...
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "talon-cli-images-"));
  const paths: string[] = [];
