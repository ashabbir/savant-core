...
export function buildSandboxCreateArgs(params: {
  ...
  args.push("--label", "talon.sandbox=1");
  args.push("--label", `talon.sessionKey=${params.scopeKey}`);
  args.push("--label", `talon.createdAtMs=${createdAtMs}`);
  if (params.configHash) {
    args.push("--label", `talon.configHash=${params.configHash}`);
  }
...
async function readContainerConfigHash(containerName: string): Promise<string | null> {
  ...
  return await readLabel("talon.configHash");
}

function formatSandboxRecreateHint(params: { scope: SandboxConfig["scope"]; sessionKey: string }) {
  if (params.scope === "session") {
    return formatCliCommand(`talon sandbox recreate --session ${params.sessionKey}`);
  }
  if (params.scope === "agent") {
    const agentId = resolveSandboxAgentId(params.sessionKey) ?? "main";
    return formatCliCommand(`talon sandbox recreate --agent ${agentId}`);
  }
  return formatCliCommand("talon sandbox recreate --all");
}
