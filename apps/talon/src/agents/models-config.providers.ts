...
type ModelsConfig = NonNullable<TalonConfig["models"]>;
...
function normalizeOllamaOpenAiBaseUrl(rawUrl: string | undefined): string {
...
const OLLAMA_BASE_URL = normalizeOllamaOpenAiBaseUrl(process.env.OLLAMA_BASE_URL);
const OLLAMA_API_BASE_URL =
  process.env.OLLAMA_API_BASE_URL || OLLAMA_BASE_URL.replace(/\/v1$/, "") || "http://127.0.0.1:11434";
...
export async function resolveImplicitProviders(params: {
  ...
  // Ollama provider - allow implicit discovery
  const ollamaProfiles = listProfilesForProvider(authStore, "ollama");
  const firstOllamaProfile = ollamaProfiles.length > 0 ? authStore.profiles[ollamaProfiles[0]] : null;
  const ollamaKey =
    resolveEnvApiKeyVarName("ollama") ??
    (firstOllamaProfile?.type === "api_key" ? firstOllamaProfile.key : undefined);

  const ollamaBaseUrl = normalizeOllamaOpenAiBaseUrl(
    (firstOllamaProfile?.type === "api_key" ? (firstOllamaProfile as any).metadata?.baseUrl : undefined) ??
      OLLAMA_BASE_URL,
  );

  const ollamaConfig = await buildOllamaProvider();
  if (ollamaConfig.models && ollamaConfig.models.length > 0) {
    providers.ollama = { ...ollamaConfig, baseUrl: ollamaBaseUrl, apiKey: ollamaKey || "ollama" };
  } else if (ollamaKey || ollamaBaseUrl !== OLLAMA_BASE_URL) {
    // If key or custom URL is present but discovery failed, still add it
    providers.ollama = { ...ollamaConfig, baseUrl: ollamaBaseUrl, apiKey: ollamaKey || "ollama" };
  }
...
export async function resolveImplicitCopilotProvider(params: {
  ...
  // Our Copilot auth lives in Talon's auth-profiles store instead, so we also
  // write a runtime-only auth.json entry for pi-coding-agent to pick up.
  //
  // This is safe because it's (1) within Talon's agent dir, (2) contains the
  // GitHub token (not the exchanged Copilot token), and (3) matches existing
  // patterns for OAuth-like providers in pi-coding-agent.
  // Note: we deliberately do not write pi-coding-agent's `auth.json` here.
  // Talon uses its own auth store and exchanges tokens at runtime.
  // `models list` uses Talon's auth heuristics for availability.
