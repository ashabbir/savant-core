...
export async function resolveApiKeyForProvider(params: {
  ...
  const findApiKey = async (targetStore: AuthProfileStore, targetAgentDir?: string) => {
    ...
    for (const candidate of order) {
        try {
            ...
            if (resolved) {
                const mode = targetStore.profiles[candidate]?.type;
                return {
                    apiKey: resolved.apiKey,
                    profileId: candidate,
                    source: `profile:${candidate}`,
                    mode: mode === "oauth" ? "oauth" : mode === "token" ? "token" : "api-key",
                } as ResolvedProviderAuth;
            }
        } catch {}
    }
    return null;
  ...
  // 1. Try agent-specific store
  const agentAuth = await findApiKey(store, params.agentDir);
  if (agentAuth) return agentAuth;

  // 2. Fallback to main store if we're in a sub-agent
  const authPath = resolveAuthStorePath(params.agentDir);
  const mainAuthPath = resolveAuthStorePath();
  if (params.agentDir && authPath !== mainAuthPath) {
      const mainStore = ensureAuthProfileStore();
      const mainAuth = await findApiKey(mainStore, undefined);
      if (mainAuth) return mainAuth;
  }
  ...
  throw new Error(
    [
      ...
      `Configure auth for this agent (${formatCliCommand("talon agents add <id>")}) or copy auth-profiles.json from the main agentDir.`,
    ].join(" "),
  );
}
