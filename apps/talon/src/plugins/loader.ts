const createJiti = (_url: string, _opts: any) => (path: string) => { throw new Error(`jiti missing, cannot load ${path}`); };
...
import { discoverTalonPlugins } from "./discovery.js";
...
function resolvePluginModuleExport(moduleExport: unknown): {
  definition?: TalonPluginDefinition;
  register?: TalonPluginDefinition["register"];
} {
  const resolved =
    moduleExport &&
  ...
  if (typeof resolved === "function") {
    return {
      register: resolved as TalonPluginDefinition["register"],
    };
  }
  if (resolved && typeof resolved === "object") {
    const def = resolved as TalonPluginDefinition;
...
export function loadTalonPlugins(options: PluginLoadOptions = {}): PluginRegistry {
  ...
  const discovery = discoverTalonPlugins({
    workspaceDir: options.workspaceDir,
  ...
  const jiti = createJiti(import.meta.url, {
    ...
    ...(pluginSdkAlias
      ? {
        alias: { "talon/plugin-sdk": pluginSdkAlias },
      }
      : {}),
  });
  ...
  for (const candidate of discovery.candidates) {
    ...
    let mod: TalonPluginModule | null = null;
    try {
      mod = jiti(candidate.source) as TalonPluginModule;
    } catch (err) {
