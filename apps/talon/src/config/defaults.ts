...
function resolveAnthropicDefaultAuthMode(cfg: TalonConfig): AnthropicAuthDefaultsMode | null {
...
export function applyMessageDefaults(cfg: TalonConfig): TalonConfig {
...
export function applyTalkApiKey(config: TalonConfig): TalonConfig {
...
export function applyModelDefaults(cfg: TalonConfig): TalonConfig {
  let mutated = false;
  let nextCfg = cfg;
...
export function applyAgentDefaults(cfg: TalonConfig): TalonConfig {
  ...
  // Force model override - REMOVED

  // Force update existing agents - REMOVED
  if (agents?.list) {
    // Check if actually changed to avoid unnecessary mutation flag if already set (though we force it here)
    // For simplicity, just set it.
    // ...
  }

  if (process.env.TALON_DEFAULT_AGENT_ID === "gemini-cli") {
    const list = agents?.list ? [...agents.list] : [];
    if (!list.some((a) => a.id === "gemini-cli")) {
      list.push({
        id: "gemini-cli",
        default: true,
...
export function applyLoggingDefaults(cfg: TalonConfig): TalonConfig {
...
export function applyContextPruningDefaults(cfg: TalonConfig): TalonConfig {
...
export function applyCompactionDefaults(cfg: TalonConfig): TalonConfig {
  const defaults = cfg.agents?.defaults;
  if (!defaults) {
    return cfg;
  }
