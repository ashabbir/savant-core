...
import type { TalonConfig, ConfigFileSnapshot, LegacyConfigIssue } from "./types.js";
import {
...
import { compareTalonVersions } from "./version.js";
...
function coerceConfig(value: unknown): TalonConfig {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    return {};
  }
  return value as TalonConfig;
}
...
function stampConfigVersion(cfg: TalonConfig): TalonConfig {
...
function warnIfConfigFromFuture(cfg: TalonConfig, logger: Pick<typeof console, "warn">): void {
  const touched = cfg.meta?.lastTouchedVersion;
  if (!touched) {
    return;
  }
  const cmp = compareTalonVersions(VERSION, touched);
  if (cmp === null) {
    return;
  }
  if (cmp < 0) {
    logger.warn(
      `Config was last written by a newer Talon (${touched}); current version is ${VERSION}.`,
    );
  }
}

function applyConfigEnv(cfg: TalonConfig, env: NodeJS.ProcessEnv): void {
...
export function createConfigIO(overrides: ConfigIoDeps = {}) {
  ...
  function loadConfig(): TalonConfig {
    try {
      ...
      if (resolved && typeof resolved === "object" && "env" in resolved) {
        applyConfigEnv(resolved as TalonConfig, deps.env);
      }
      ...
      const preValidationDuplicates = findDuplicateAgentDirs(resolvedConfig as TalonConfig, {
        env: deps.env,
  ...
  async function writeConfigFile(cfg: TalonConfig) {
    clearConfigCache();
...
// module scope. `TALON_CONFIG_PATH` (and friends) are expected to work even
...
function resolveConfigCacheMs(env: NodeJS.ProcessEnv): number {
  const raw = env.TALON_CONFIG_CACHE_MS?.trim();
  if (raw === "" || raw === "0") {
    return 0;
  }
  if (!raw) {
...
function shouldUseConfigCache(env: NodeJS.ProcessEnv): boolean {
  if (env.TALON_DISABLE_CONFIG_CACHE?.trim()) {
    return false;
  }
...
export async function writeConfigFile(cfg: TalonConfig): Promise<void> {
  clearConfigCache();
