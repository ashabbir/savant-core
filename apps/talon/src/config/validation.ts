...
import type { TalonConfig, ConfigValidationIssue } from "./types.js";
...
function validateIdentityAvatar(config: TalonConfig): ConfigValidationIssue[] {
...
export function validateConfigObject(
  raw: unknown,
): { ok: true; config: TalonConfig } | { ok: false; issues: ConfigValidationIssue[] } {
  ...
  const validated = TalonSchema.safeParse(raw);
  ...
  const duplicates = findDuplicateAgentDirs(validated.data as TalonConfig);
  ...
  const avatarIssues = validateIdentityAvatar(validated.data as TalonConfig);
  ...
  return {
    ...
    config: applyModelDefaults(
      applyAgentDefaults(applySessionDefaults(validated.data as TalonConfig)),
    ),
  };
}
...
export function validateConfigObjectWithPlugins(raw: unknown):
  ...
  const CHANNEL_IDS = listDeliverableMessageChannels();
  ...
  const validateHeartbeatTarget = (target: string | undefined, path: string) => {
    ...
    if (normalizeMessageChannel(trimmed)) {
      return;
    }
