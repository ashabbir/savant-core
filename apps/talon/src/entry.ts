...
function ensureExperimentalWarningSuppressed(): boolean {
  if (isTruthyEnvValue(process.env.TALON_NO_RESPAWN)) {
    return false;
  }
  if (isTruthyEnvValue(process.env.TALON_NODE_OPTIONS_READY)) {
    return false;
  }
  ...
  process.env.TALON_NODE_OPTIONS_READY = "1";
...
if (!ensureExperimentalWarningSuppressed()) {
  ...
  if (!parsed.ok) {
    ...
    console.error(`[talon] ${parsed.error}`);
    process.exit(2);
  }
