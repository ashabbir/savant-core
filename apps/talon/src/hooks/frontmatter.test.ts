...
`;
    ...
    expect(parsed.talon.emoji).toBe("💾");
    expect(parsed.talon.events).toEqual(["command:new"]);
  });
...
`;
    ...
    const parsed = JSON.parse(result.metadata);
    expect(parsed.talon.emoji).toBe("📝");
    expect(parsed.talon.events).toEqual(["command"]);
    expect(parsed.talon.requires.config).toEqual(["workspace.dir"]);
    expect(parsed.talon.install[0].kind).toBe("bundled");
  });

  it("handles single-line metadata (inline JSON)", () => {
    const content = `---
name: simple-hook
metadata: {"talon": {"events": ["test"]}}
---
`;
    const result = parseFrontmatter(content);
    expect(result.name).toBe("simple-hook");
    expect(result.metadata).toBe('{"talon": {"events": ["test"]}}');
  });
...
describe("resolveTalonMetadata", () => {
  it("extracts talon metadata from parsed frontmatter", () => {
    const frontmatter = {
      name: "test-hook",
    ...
    const result = resolveTalonMetadata(frontmatter);
  ...
  it("returns undefined when talon key is missing", () => {
    const frontmatter = {
  ...
  it("handles install specs", () => {
    const frontmatter = {
      metadata: JSON.stringify({
        talon: {
          events: ["command"],
          install: [
            { id: "bundled", kind: "bundled", label: "Bundled with Talon" },
            { id: "npm", kind: "npm", package: "@talon/hook" },
          ],
        },
      }),
    };
    ...
    expect(result?.install?.[1].package).toBe("@talon/hook");
  });
    ...
    "talon":
      {
        ...
        "events": ["command:new"],
        "requires": { "config": ["workspace.dir"] },
        "install": [{ "id": "bundled", "kind": "bundled", "label": "Bundled with Talon" }],
      },
  }
---
...
`;
    ...
    const talon = resolveTalonMetadata(frontmatter);
    expect(talon).toBeDefined();
    expect(talon?.emoji).toBe("💾");
    expect(talon?.events).toEqual(["command:new"]);
    expect(talon?.requires?.config).toEqual(["workspace.dir"]);
    expect(talon?.install?.[0].kind).toBe("bundled");
  });
