...
async function makeTempDir() {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "talon-hooks-e2e-"));
  tempDirs.push(dir);
  return dir;
}
...
describe("hooks install (e2e)", () => {
  ...
  beforeEach(async () => {
    ...
    prevStateDir = process.env.TALON_STATE_DIR;
    prevBundledDir = process.env.TALON_BUNDLED_HOOKS_DIR;
    process.env.TALON_STATE_DIR = path.join(baseDir, "state");
    process.env.TALON_BUNDLED_HOOKS_DIR = path.join(baseDir, "bundled-none");
    vi.resetModules();
  });
  ...
  afterEach(async () => {
    if (prevStateDir === undefined) {
      ...
      process.env.TALON_STATE_DIR = prevStateDir;
    }

    if (prevBundledDir === undefined) {
      delete process.env.TALON_BUNDLED_HOOKS_DIR;
    } else {
      process.env.TALON_BUNDLED_HOOKS_DIR = prevBundledDir;
    }
  ...
  it("installs a hook pack and triggers the handler", async () => {
    ...
    await fs.writeFile(
      ...
      [
        ...
        'metadata: {"talon":{"events":["command:new"]}}',
        "---",
        "",
        "# Hello Hook",
        "",
