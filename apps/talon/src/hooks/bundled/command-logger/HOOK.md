...
homepage: https://docs.talon.ai/hooks#command-logger
metadata:
  {
    "talon":
      {
...
2. **Appends to log file** - Writes a JSON line to `~/.talon/logs/commands.log`
...
1. **Manual rotation**:
   ...
   ```bash
   mv ~/.talon/logs/commands.log ~/.talon/logs/commands.log.old
   ```

2. **Use logrotate** (Linux):
   Create `/etc/logrotate.d/talon`:
   ```
   /home/username/.talon/logs/commands.log {
       weekly
       rotate 4
       compress
       missingok
