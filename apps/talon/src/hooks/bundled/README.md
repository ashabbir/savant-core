...
This directory contains hooks that ship with Talon. These hooks are automatically discovered and can be enabled/disabled via CLI or configuration.
...
**Docs**: https://docs.talon.ai/hooks/soul-evil
...
homepage: https://docs.talon.ai/hooks#my-hook
metadata:
...
Hooks can be configured in `~/.talon/talon.json`:
...
2. Restart gateway: `pkill -9 -f 'talon.*gateway' && pnpm talon gateway`
3. Enable the hook: `talon hooks enable my-hook`
4. Trigger the event (e.g., send `/new` command)
5. Check gateway logs for hook execution

## Documentation

Full documentation: https://docs.talon.ai/hooks
