...
import { resolveMessageChannel, type GatewayMessageChannel } from "../utils/message-channel.js";
...
export async function agentCommand(
  ...
  if (agentIdOverride) {
    ...
    if (!knownAgents.includes(agentIdOverride)) {
      throw new Error(
        `Unknown agent id "${agentIdOverrideRaw}". Use "${formatCliCommand("talon agents list")}" to see configured agents.`,
      );
    }
  }
  ...
  const agentConfigResolved = resolveAgentConfig(cfg, sessionAgentId);
  ...
  try {
    ...
    const disableTools = agentConfigResolved?.tools?.disableTools ?? cfg.tools?.disableTools;
    ...
    try {
      ...
      const messageChannel = resolveMessageChannel(
        ...
        (opts.replyChannel ?? opts.channel) as GatewayMessageChannel,
      );
