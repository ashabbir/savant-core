...
const SEARCH_TOOL = "See README.md";
...
export async function docsSearchCommand(queryParts: string[], runtime: RuntimeEnv) {
  ...
  if (!query) {
    const docs = formatDocsLink("/", "docs.talon.ai");
    if (isRich()) {
      runtime.log(`${theme.muted("Docs:")} ${docs}`);
      runtime.log(`${theme.muted("Search:")} ${formatCliCommand('talon docs "your query"')}`);
    } else {
      runtime.log("Docs: See README.md");
      runtime.log(`Search: ${formatCliCommand('talon docs "your query"')}`);
    }
    return;
  }
