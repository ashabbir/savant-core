...
import type { TalonConfig, GatewayBindMode } from "../config/config.js";
...
export async function noteSecurityWarnings(cfg: TalonConfig) {
  const warnings: string[] = [];
  const auditHint = `- Run: ${formatCliCommand("talon security audit --deep")}`;
    ...
    if (!hasSharedSecret) {
      const authFixLines =
        resolvedAuth.mode === "password"
          ? [
              `  Fix: ${formatCliCommand("talon configure")} to set a password`,
              `  Or switch to token: ${formatCliCommand("talon config set gateway.auth.mode token")}`,
            ]
          : [
              `  Fix: ${formatCliCommand("talon doctor --fix")} to generate a token`,
      ...
      warnings.push(
        ...
        `  Fix: ${formatCliCommand("talon config set gateway.bind loopback")}`,
        ...authFixLines,
      );
    } else {
