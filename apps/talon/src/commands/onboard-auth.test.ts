...
const requireAgentDir = () => {
  const agentDir = process.env.TALON_AGENT_DIR;
  if (!agentDir) {
    throw new Error("TALON_AGENT_DIR not set");
  }
  return agentDir;
};
...
describe("writeOAuthCredentials", () => {
  ...
  afterEach(async () => {
    ...
    delete process.env.TALON_OAUTH_DIR;
  });

  it("writes auth-profiles.json under TALON_AGENT_DIR when set", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "talon-oauth-"));
    process.env.TALON_STATE_DIR = tempStateDir;
    process.env.TALON_AGENT_DIR = path.join(tempStateDir, "agent");
    process.env.PI_CODING_AGENT_DIR = process.env.TALON_AGENT_DIR;
...
describe("setMinimaxApiKey", () => {
  ...
  it("writes to TALON_AGENT_DIR when set", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "talon-minimax-"));
    process.env.TALON_STATE_DIR = tempStateDir;
    process.env.TALON_AGENT_DIR = path.join(tempStateDir, "custom-agent");
    process.env.PI_CODING_AGENT_DIR = process.env.TALON_AGENT_DIR;
