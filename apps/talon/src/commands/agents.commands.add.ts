...
import { DEFAULT_AGENT_ID, normalizeAgentId } from "../agents/session-key.js";
...
export async function agentsAddCommand(
  ...
  if (nonInteractive) {
    ...
    if (findAgentEntryIndex(listAgentEntries(cfg), agentId as string) >= 0) {
  ...
  try {
    await prompter.intro("Add Talon agent");
    const name =
      nameInput ??
      (await prompter.text({
        message: "Agent name",
