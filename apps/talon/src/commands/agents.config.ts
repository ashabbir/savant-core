...
export function listAgentEntries(cfg: TalonConfig): AgentEntry[] {
...
function resolveAgentName(cfg: TalonConfig, agentId: string) {
...
function resolveAgentModel(cfg: TalonConfig, agentId: string) {
  const entry = listAgentEntries(cfg).find(
    (agent) => normalizeAgentId(agent.id) === normalizeAgentId(agentId),
  );
  if (entry?.model) {
...
export function buildAgentSummaries(cfg: TalonConfig): AgentSummary[] {
