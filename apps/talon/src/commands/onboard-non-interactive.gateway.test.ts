...
describe("onboard (non-interactive): gateway and remote auth", () => {
  const prev = {
    ...
    password: process.env.TALON_GATEWAY_PASSWORD,
  };
  ...
  const initStateDir = async (prefix: string) => {
    ...
    process.env.TALON_STATE_DIR = stateDir;
  ...
  beforeAll(async () => {
    ...
    delete process.env.TALON_GATEWAY_PASSWORD;

    tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "talon-onboard-"));
  ...
  afterAll(async () => {
    ...
    process.env.TALON_GATEWAY_PASSWORD = prev.password;
  });
  ...
  it("writes gateway token auth into config and gateway enforces it", async () => {
    ...
    const workspace = path.join(stateDir, "talon");
  ...
  it("auto-generates token auth when binding LAN and persists the token", async () => {
    ...
    process.env.TALON_STATE_DIR = stateDir;
    process.env.TALON_CONFIG_PATH = path.join(stateDir, "talon.json");
