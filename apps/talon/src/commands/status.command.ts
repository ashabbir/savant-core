...
export async function statusCommand(
  ...
  runtime.log(theme.heading("Talon status"));
  runtime.log("");
  ...
  runtime.log(theme.muted(`Full report: ${formatCliCommand("talon security audit")}`));
  runtime.log(theme.muted(`Deep probe: ${formatCliCommand("talon security audit --deep")}`));
  ...
  runtime.log("");
  runtime.log("FAQ: See README.md");
  runtime.log("Troubleshooting: See README.md");
  runtime.log("");
  ...
  runtime.log(`  Need to share?      ${formatCliCommand("talon status --all")}`);
  runtime.log(`  Need to debug live? ${formatCliCommand("talon logs --follow")}`);
  if (gatewayReachable) {
    runtime.log(`  Need to test channels? ${formatCliCommand("talon status --deep")}`);
  } else {
    runtime.log(`  Fix reachability first: ${formatCliCommand("talon gateway probe")}`);
  }
}
