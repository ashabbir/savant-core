...
import { readConfigFileSnapshot, type TalonConfig } from "../../config/config.js";
...
function applyDefaultModel(cfg: TalonConfig, model: string): TalonConfig {
...
export async function modelsAuthLoginCommand(opts: LoginOptions, runtime: RuntimeEnv) {
  ...
  if (providers.length === 0) {
    throw new Error(
      `No provider plugins found. Install one via \`${formatCliCommand("talon plugins install")}\`.`,
    );
  }
