...
export async function modelsStatusCommand(
  ...
  const agentDir = agentId ? resolveAgentDir(cfg, agentId) : resolveTalonAgentDir();
  ...
  if (missingProvidersInUse.length > 0) {
    ...
    for (const provider of missingProvidersInUse) {
      const hint =
        provider === "anthropic"
          ? `Run \`claude setup-token\`, then \`${formatCliCommand("talon models auth setup-token")}\` or \`${formatCliCommand("talon configure")}\`.`
          : `Run \`${formatCliCommand("talon configure")}\` or set an API key env var.`;
