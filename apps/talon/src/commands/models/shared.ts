...
export async function updateConfig(
  mutator: (cfg: TalonConfig) => TalonConfig,
): Promise<TalonConfig> {
...
export function resolveModelTarget(params: { raw: string; cfg: TalonConfig }): {
  provider: string;
  model: string;
} {
...
export function buildAllowlistSet(cfg: TalonConfig): Set<string> {
...
export function resolveKnownAgentId(params: {
  ...
  if (!knownAgents.includes(agentId)) {
    throw new Error(
      `Unknown agent id "${raw}". Use "${formatCliCommand("talon agents list")}" to see configured agents.`,
    );
  }
  return agentId;
}
