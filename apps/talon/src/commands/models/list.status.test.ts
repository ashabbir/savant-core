...
const mocks = vi.hoisted(() => {
  ...
  return {
    store,
    resolveTalonAgentDir: vi.fn().mockReturnValue("/tmp/talon-agent"),
    resolveAgentDir: vi.fn().mockReturnValue("/tmp/talon-agent"),
    ...
    resolveAuthStorePathForDisplay: vi
      .fn()
      .mockReturnValue("/tmp/talon-agent/auth-profiles.json"),
...
vi.mock("../../agents/agent-paths.js", () => ({
  resolveTalonAgentDir: mocks.resolveTalonAgentDir,
}));
...
describe("modelsStatusCommand auth overview", () => {
  it("includes masked auth sources in JSON output", async () => {
    ...
    expect(mocks.resolveTalonAgentDir).toHaveBeenCalled();
    expect(payload.defaultModel).toBe("anthropic/claude-opus-4-5");
    expect(payload.auth.storePath).toBe("/tmp/talon-agent/auth-profiles.json");
  ...
  it("uses agent overrides and reports sources", async () => {
    ...
    mocks.resolveAgentDir.mockReturnValue("/tmp/talon-agent-custom");
    ...
    try {
      ...
      expect(payload.agentDir).toBe("/tmp/talon-agent-custom");
