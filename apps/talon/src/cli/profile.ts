...
function resolveProfileStateDir(profile: string, homedir: () => string): string {
  ...
  return path.join(homedir(), `.talon${suffix}`);
}
...
export function applyCliProfileEnv(params: {
  ...
  env.TALON_PROFILE = profile;

  const stateDir = env.TALON_STATE_DIR?.trim() || resolveProfileStateDir(profile, homedir);
  if (!env.TALON_STATE_DIR?.trim()) {
    env.TALON_STATE_DIR = stateDir;
  }

  if (!env.TALON_CONFIG_PATH?.trim()) {
    env.TALON_CONFIG_PATH = path.join(stateDir, "talon.json");
  }

  if (profile === "dev" && !env.TALON_GATEWAY_PORT?.trim()) {
    env.TALON_GATEWAY_PORT = "19001";
  }
}
