...
function renderNodeServiceStartHints(): string[] {
  const base = [formatCliCommand("talon node install"), formatCliCommand("talon node start")];
...
export async function runNodeDaemonInstall(opts: NodeDaemonInstallOptions) {
  ...
  if (loaded && !opts.force) {
    ...
    if (!json) {
      ...
      defaultRuntime.log(`Reinstall with: ${formatCliCommand("talon node install --force")}`);
    }
    return;
  }
...
export async function runNodeDaemonStatus(opts: NodeDaemonStatusOptions = {}) {
  ...
  const hintEnv = {
    ...baseEnv,
    TALON_LOG_PREFIX: baseEnv.TALON_LOG_PREFIX ?? "node",
