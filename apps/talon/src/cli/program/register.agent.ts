...
import { chatCommand } from "../../commands/agent/chat.js";
...
${formatHelpExamples([
          ['talon agent --to +15555550123 --message "status update"', "Start a new session."],
          ['talon agent --agent ops --message "Summarize logs"', "Use a specific agent."],
          [
            'talon agent --session-id 1234 --message "Summarize inbox" --thinking medium',
            "Target a session with explicit thinking level.",
          ],
          [
            'talon agent --to +15555550123 --message "Trace logs" --verbose on --json',
            "Enable verbose logging and JSON output.",
          ],
          ['talon agent --to +15555550123 --message "Summon reply" --deliver', "Deliver reply."],
          [
            'talon agent --agent ops --message "Generate report" --deliver --reply-channel slack --reply-to "#reports"',
            "Send reply to a different channel/target.",
          ],
        ])}

${theme.muted("Docs:")} ${formatDocsLink("/cli/agent", "docs.talon.ai/cli/agent")}`,
    )
    .action(async (opts) => {
      if (!opts.message) {
        await runCommandWithRuntime(defaultRuntime, async () => {
          await chatCommand({ agent: opts.agent, local: Boolean(opts.local) });
        });
        return;
      }
  ...
  program
    ...
    .option("--local", "Run the embedded agent locally", true)
    .action(async (opts) => {
      await runCommandWithRuntime(defaultRuntime, async () => {
  ...
  const agents = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/agents", "docs.talon.ai/cli/agents")}\n`,
    );
...
${theme.heading("Examples:")}
${formatHelpExamples([
          ['talon agents set-identity --agent main --name "Talon" --emoji "🦅"', "Set name + emoji."],
          ["talon agents set-identity --agent main --avatar avatars/talon.png", "Set avatar path."],
          [
            "talon agents set-identity --workspace ~/.talon/workspace --from-identity",
            "Load from IDENTITY.md.",
          ],
          [
            "talon agents set-identity --identity-file ~/.talon/workspace/IDENTITY.md --agent main",
