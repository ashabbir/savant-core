...
import { resolveTalonPackageRoot } from "../infra/talon-root.js";
...
const STEP_LABELS: Record<string, string> = {
  ...
  "talon doctor entry": "Checking doctor entrypoint",
  "talon doctor": "Running doctor checks",
...
const DEFAULT_PACKAGE_NAME = "talon";
const CORE_PACKAGE_NAMES = new Set([DEFAULT_PACKAGE_NAME]);
const CLI_NAME = resolveCliName();
const TALON_REPO_URL = "https://github.com/talon/talon.git";
const DEFAULT_GIT_DIR = path.join(os.homedir(), ".talon");
...
function normalizeTag(value?: string | null): string | null {
  ...
  if (trimmed.startsWith("talon@")) {
    return trimmed.slice("talon@".length);
  }
...
async function tryWriteCompletionCache(root: string, jsonMode: boolean): Promise<void> {
  const binPath = path.join(root, "talon.mjs");
  ...
  if (!status.profileInstalled) {
    ...
    if (isCancel(shouldInstall) || !shouldInstall) {
      if (!opts.skipPrompt) {
        defaultRuntime.log(
          theme.muted(
            `Skipped. Run \`${replaceCliName(formatCliCommand("talon completion --install"), CLI_NAME)}\` later to enable.`,
          ),
        );
      }
      return;
...
function resolveGitInstallDir(): string {
  const override = process.env.TALON_GIT_DIR?.trim();
  if (override) {
...
async function ensureGitCheckout(params: {
  ...
  if (!dirExists) {
    return await runUpdateStep({
      name: "git clone",
      argv: ["git", "clone", TALON_REPO_URL, params.dir],
      timeoutMs: params.timeoutMs,
  ...
  if (!(await isGitCheckout(params.dir))) {
    ...
    if (!empty) {
      throw new Error(
        `TALON_GIT_DIR points at a non-git directory: ${params.dir}. Set TALON_GIT_DIR to an empty folder or an talon checkout.`,
      );
    }
    return await runUpdateStep({
      name: "git clone",
  ...
  if (!(await isCorePackage(params.dir))) {
    throw new Error(`TALON_GIT_DIR does not look like a core checkout: ${params.dir}.`);
  }
...
export async function updateStatusCommand(opts: UpdateStatusOptions): Promise<void> {
  ...
  defaultRuntime.log(theme.heading("Talon update status"));
  defaultRuntime.log("");
  defaultRuntime.log(
    renderTable({
      width: tableWidth,
...
export async function updateCommand(opts: UpdateCommandOptions): Promise<void> {
  ...
  if (!opts.json) {
    defaultRuntime.log(theme.heading("Updating Talon..."));
    defaultRuntime.log("");
  }
  ...
  if (result.status === "skipped") {
    ...
    if (result.reason === "not-git-install") {
      defaultRuntime.log(
        theme.warn(
          `Skipped: this Talon install isn't a git checkout, and the package manager couldn't be detected. Update via your package manager, then run \`${replaceCliName(formatCliCommand("talon doctor"), CLI_NAME)}\` and \`${replaceCliName(formatCliCommand("talon gateway restart"), CLI_NAME)}\`.`,
        ),
      );
      defaultRuntime.log(
        theme.muted(
          `Examples: \`${replaceCliName("npm i -g talon@latest", CLI_NAME)}\` or \`${replaceCliName("pnpm add -g talon@latest", CLI_NAME)}\``,
        ),
      );
    }
    defaultRuntime.exit(0);
  ...
  if (shouldRestart) {
    ...
    try {
      ...
      if (!opts.json && restarted) {
        ...
        defaultRuntime.log("");
        process.env.TALON_UPDATE_IN_PROGRESS = "1";
        try {
          ...
          delete process.env.TALON_UPDATE_IN_PROGRESS;
        }
      }
    } catch (err) {
      if (!opts.json) {
        ...
        defaultRuntime.log(
          theme.muted(
            `You may need to restart the service manually: ${replaceCliName(formatCliCommand("talon gateway restart"), CLI_NAME)}`,
          ),
        );
      }
    }
    ...
    if (result.mode === "npm" || result.mode === "pnpm") {
      defaultRuntime.log(
        theme.muted(
          `Tip: Run \`${replaceCliName(formatCliCommand("talon doctor"), CLI_NAME)}\`, then \`${replaceCliName(formatCliCommand("talon gateway restart"), CLI_NAME)}\` to apply updates to a running gateway.`,
        ),
      );
    } else {
      defaultRuntime.log(
        theme.muted(
          `Tip: Run \`${replaceCliName(formatCliCommand("talon gateway restart"), CLI_NAME)}\` to apply updates to a running gateway.`,
        ),
      );
    }
  }
...
export async function updateWizardCommand(opts: UpdateWizardOptions = {}): Promise<void> {
  if (!process.stdin.isTTY) {
    defaultRuntime.error(
      "Update wizard requires a TTY. Use `talon update --channel <stable|beta|dev>` instead.",
    );
    defaultRuntime.exit(1);
    return;
  }
  ...
  if (requestedChannel === "dev" && updateStatus.installKind !== "git") {
    ...
    if (!hasGit) {
      ...
      if (dirExists) {
        ...
        if (!empty) {
          defaultRuntime.error(
            `TALON_GIT_DIR points at a non-git directory: ${gitDir}. Set TALON_GIT_DIR to an empty folder or an talon checkout.`,
          );
          defaultRuntime.exit(1);
          return;
        }
      ...
      const ok = await confirm({
        message: stylePromptMessage(
          `Create a git checkout at ${gitDir}? (override via TALON_GIT_DIR)`,
        ),
        initialValue: true,
      });
      if (isCancel(ok) || !ok) {
...
export function registerUpdateCli(program: Command) {
  const update = program
    .command("update")
    .description("Update Talon to the latest version")
    ...
    .addHelpText("after", () => {
      const examples = [
        ["talon update", "Update a source checkout (git)"],
        ["talon update --channel beta", "Switch to beta channel (git + npm)"],
        ["talon update --channel dev", "Switch to dev channel (git + npm)"],
        ["talon update --tag beta", "One-off update to a dist-tag or version"],
        ["talon update --no-restart", "Update without restarting the service"],
        ["talon update --json", "Output result as JSON"],
        ["talon update --yes", "Non-interactive (accept downgrade prompts)"],
        ["talon update wizard", "Interactive update wizard"],
        ["talon --update", "Shorthand for talon update"],
      ] as const;
...
${theme.heading("Switch channels:")}
  ...
  - Run talon update status to see the active channel and source
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/update", "docs.talon.ai/cli/update")}`;
    })
    .action(async (opts) => {
      try {
        await updateCommand({
  ...
  update
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["talon update status", "Show channel + version status."],
          ["talon update status --json", "JSON output."],
          ["talon update status --timeout 10", "Custom timeout."],
        ])}\n\n${theme.heading("Notes:")}\n${theme.muted(
          "- Shows current update channel (stable/beta/dev) and source",
        )}\n${theme.muted("- Includes git tag/branch/SHA for source checkouts")}\n\n${theme.muted(
          "Docs:",
        )} ${formatDocsLink("/cli/update", "docs.talon.ai/cli/update")}`,
    )
    .action(async (opts) => {
      try {
