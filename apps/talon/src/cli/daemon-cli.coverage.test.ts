...
describe("daemon-cli coverage", () => {
  const originalEnv = {
    TALON_STATE_DIR: process.env.TALON_STATE_DIR,
    TALON_CONFIG_PATH: process.env.TALON_CONFIG_PATH,
    TALON_GATEWAY_PORT: process.env.TALON_GATEWAY_PORT,
    TALON_PROFILE: process.env.TALON_PROFILE,
  };

  beforeEach(() => {
    process.env.TALON_STATE_DIR = "/tmp/talon-cli-state";
    process.env.TALON_CONFIG_PATH = "/tmp/talon-cli-state/talon.json";
    delete process.env.TALON_GATEWAY_PORT;
    delete process.env.TALON_PROFILE;
    serviceReadCommand.mockResolvedValue(null);
  });

  afterEach(() => {
    if (originalEnv.TALON_STATE_DIR !== undefined) {
      process.env.TALON_STATE_DIR = originalEnv.TALON_STATE_DIR;
    } else {
      delete process.env.TALON_STATE_DIR;
    }

    if (originalEnv.TALON_CONFIG_PATH !== undefined) {
      process.env.TALON_CONFIG_PATH = originalEnv.TALON_CONFIG_PATH;
    } else {
      delete process.env.TALON_CONFIG_PATH;
    }

    if (originalEnv.TALON_GATEWAY_PORT !== undefined) {
      process.env.TALON_GATEWAY_PORT = originalEnv.TALON_GATEWAY_PORT;
    } else {
      delete process.env.TALON_GATEWAY_PORT;
    }

    if (originalEnv.TALON_PROFILE !== undefined) {
      process.env.TALON_PROFILE = originalEnv.TALON_PROFILE;
    } else {
      delete process.env.TALON_PROFILE;
    }
  });
  ...
  it("derives probe URL from service args + env (json)", async () => {
    ...
    serviceReadCommand.mockResolvedValueOnce({
      ...
      environment: {
        TALON_PROFILE: "dev",
        TALON_STATE_DIR: "/tmp/talon-daemon-state",
        TALON_CONFIG_PATH: "/tmp/talon-daemon-state/talon.json",
        TALON_GATEWAY_PORT: "19001",
      },
