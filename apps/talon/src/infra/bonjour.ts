...
function isDisabledByEnv() {
  if (isTruthyEnvValue(process.env.TALON_DISABLE_BONJOUR)) {
    return true;
  }
...
function safeServiceName(name: string) {
  ...
  return trimmed.length > 0 ? trimmed : "Talon";
}

function prettifyInstanceName(name: string) {
  const normalized = name.trim().replace(/\s+/g, " ");
  return normalized.replace(/\s+\(Talon\)\s*$/i, "").trim() || normalized;
}
...
export async function startGatewayBonjourAdvertiser(
  ...
  let Protocol: any;
  let getResponder: any;
  try {
    const ciao = await import("@homebridge/ciao");
    Protocol = ciao.Protocol;
    getResponder = ciao.getResponder;
  } catch {
    getLogger().warn("bonjour: optional dependency @homebridge/ciao not found, skipping advertisement");
    return { stop: async () => { } };
  }
  ...
  const hostnameRaw =
    process.env.TALON_MDNS_HOSTNAME?.trim() ||
    process.env.TALON_MDNS_HOSTNAME?.trim() ||
    "talon";
  const hostname =
    hostnameRaw
