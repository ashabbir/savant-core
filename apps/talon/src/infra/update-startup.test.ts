...
vi.mock("./talon-root.js", () => ({
  resolveTalonPackageRoot: vi.fn(),
}));
...
describe("update-startup", () => {
  ...
  beforeEach(async () => {
    ...
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "talon-update-check-"));
  ...
  it("logs update hint for npm installs when newer tag exists", async () => {
    const { resolveTalonPackageRoot } = await import("./talon-root.js");
    ...
    vi.mocked(resolveTalonPackageRoot).mockResolvedValue("/opt/talon");
