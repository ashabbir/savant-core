...
export async function runGatewayUpdate(opts: UpdateRunnerOptions = {}): Promise<UpdateRunResult> {
  ...
  if (gitRoot && pkgRoot && path.resolve(gitRoot) === path.resolve(pkgRoot)) {
    ...
    if (channel === "dev") {
      ...
      const preflightRoot = await fs.mkdtemp(path.join(os.tmpdir(), "talon-update-preflight-"));
    ...
    const doctorEntry = path.join(gitRoot, "talon.mjs");
    ...
    const doctorStep = await runStep(
      step("talon doctor", doctorArgv, gitRoot, { TALON_UPDATE_IN_PROGRESS: "1" }),
    );
