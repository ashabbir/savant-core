...
function candidateBinDirs(opts: EnsureTalonPathOpts): string[] {
  ...
  // Bundled macOS app: `talon` lives next to the executable (process.execPath).
  try {
    const execDir = path.dirname(execPath);
    const siblingCli = path.join(execDir, "talon");
  ...
  // Project-local installs (best effort): if a `node_modules/.bin/talon` exists near cwd,
  // include it. This helps when running under launchd or other minimal PATH environments.
  const localBinDir = path.join(cwd, "node_modules", ".bin");
  if (isExecutable(path.join(localBinDir, "talon"))) {
...
/**
 * Best-effort PATH bootstrap so skills that require the `talon` CLI can run
 * under launchd/minimal environments (and inside the macOS app bundle).
 */
export function ensureTalonCliOnPath(opts: EnsureTalonPathOpts = {}) {
  if (isTruthyEnvValue(process.env.TALON_PATH_BOOTSTRAPPED)) {
    return;
  }
  process.env.TALON_PATH_BOOTSTRAPPED = "1";
