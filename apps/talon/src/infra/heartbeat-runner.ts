...
import { normalizeAgentId, toAgentStoreSessionKey } from "../agents/session-key.js";
...
function resolveActiveHoursTimezone(cfg: TalonConfig, raw?: string): string {
...
export type HeartbeatRunner = {
  stop: () => void;
  updateConfig: (cfg: TalonConfig) => void;
};

function hasExplicitHeartbeatAgents(cfg: TalonConfig) {
  const list = cfg.agents?.list ?? [];
  return list.some((entry) => Boolean(entry?.heartbeat));
}

export function isHeartbeatEnabledForAgent(cfg: TalonConfig, agentId?: string): boolean {
...
function resolveHeartbeatAgents(cfg: TalonConfig): HeartbeatAgent[] {
  const list = cfg.agents?.list ?? [];
...
export function resolveHeartbeatPrompt(cfg: TalonConfig, heartbeat?: HeartbeatConfig) {
  return resolveHeartbeatPromptText(heartbeat?.prompt ?? cfg.agents?.defaults?.heartbeat?.prompt);
}

function resolveHeartbeatAckMaxChars(cfg: TalonConfig, heartbeat?: HeartbeatConfig) {
  return Math.max(
    0,
...
export async function runHeartbeatOnce(opts: {
  ...
  const maybeSendHeartbeatOk = async () => {
    ...
    /* Channel readiness check removed */
...
export function startHeartbeatRunner(opts: {
  ...
  const updateConfig = (cfg: TalonConfig) => {
    if (state.stopped) {
      return;
    }
    const now = Date.now();
