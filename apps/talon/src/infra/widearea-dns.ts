...
export function resolveWideAreaDiscoveryDomain(params?: {
  ...
  const candidate = params?.configDomain ?? env.TALON_WIDE_AREA_DOMAIN ?? null;
...
function extractContentHash(zoneText: string): string | null {
  const match = zoneText.match(/^\s*;\s*talon-content-hash:\s*(\S+)\s*$/m);
  return match?.[1] ?? null;
}
...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  const hostname = os.hostname().split(".")[0] ?? "talon";
  const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "talon");
  const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "talon-gw");
  ...
  records.push(`_talon-gw._tcp IN PTR ${instanceLabel}._talon-gw._tcp`);
  records.push(`${instanceLabel}._talon-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
  records.push(`${instanceLabel}._talon-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
  ...
  return `; talon-content-hash: ${contentHash}\n${contentBody}`;
}
