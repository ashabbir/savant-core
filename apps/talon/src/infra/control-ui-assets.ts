...
import { resolveTalonPackageRoot, resolveTalonPackageRootSync } from "./talon-root.js";
...
export async function resolveControlUiDistIndexPath(
  ...
  const packageRoot = await resolveTalonPackageRoot({ argv1: normalized });
  if (packageRoot) {
    return path.join(packageRoot, "dist", "control-ui", "index.html");
  }

  // Fallback: traverse up and find package.json with name "talon" + dist/control-ui/index.html
...
export function resolveControlUiRootSync(opts: ControlUiRootResolveOptions = {}): string | null {
  ...
  const packageRoot = resolveTalonPackageRootSync({
    argv1,
