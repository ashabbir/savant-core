...
export function triggerTalonRestart(): RestartAttempt {
  ...
  if (process.platform !== "darwin") {
    if (process.platform === "linux") {
      const unit = normalizeSystemdUnit(
        process.env.TALON_SYSTEMD_UNIT,
        process.env.TALON_PROFILE,
      );
  ...
  const label =
    process.env.TALON_LAUNCHD_LABEL ||
    resolveGatewayLaunchAgentLabel(process.env.TALON_PROFILE);
