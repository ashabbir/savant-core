  ...
  try {
    ...
    try {
      ...
      try {
        ...
        warnings.push(
          `Rollback failed; set TALON_STATE_DIR=${targetDir} to avoid split state: ${String(rollbackErr)}`,
        );
...
async function migrateLegacySessions(
  ...
  if (!merged[mainKey]) {
    ...
    if (latest?.sessionId) {
      ...
      changes.push(`Migrated latest session → ${mainKey}`);
    }
  }
...
export async function autoMigrateState(params: {
  cfg: TalonConfig;
  env?: NodeJS.ProcessEnv;
  homedir?: () => string;
  log?: MigrationLogger;
  ...
  const stateDirRes = await autoMigrateLegacyStateDir(params);
  const detected = await detectLegacyStateMigrations(params);

  if (!detected.preview.length) {
    return {
      migrated: stateDirRes.migrated,
      skipped: false,
      changes: stateDirRes.changes,
      warnings: stateDirRes.warnings,
    };
  }
  ...
  return {
    migrated: true,
    skipped: false,
    changes: [...stateDirRes.changes, ...sessions.changes, ...agentDir.changes],
    warnings: [...stateDirRes.warnings, ...sessions.warnings, ...agentDir.warnings],
  };
}

export const autoMigrateLegacyState = autoMigrateState;
export const autoMigrateLegacyAgentDir = migrateLegacyAgentDir;
export const runLegacyStateMigrations = async () => ({ changes: [], warnings: [] });

export async function backgroundAutoMigrateState(params: {
  cfg: TalonConfig;
  env?: NodeJS.ProcessEnv;
  homedir?: () => string;
}) {
  const log = createSubsystemLogger("migration");
  try {
    const result = await autoMigrateState({ ...params, log });
    if (result.migrated) {
    ...
    log.error(`Auto-migration failed: ${String(err)}`);
  }
}
