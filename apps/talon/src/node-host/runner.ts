...
const execHostEnforced = process.env.TALON_NODE_EXEC_HOST?.trim().toLowerCase() === "app";
const execHostFallbackAllowed =
  process.env.TALON_NODE_EXEC_FALLBACK?.trim().toLowerCase() !== "0";
...
function ensureNodePathEnv(): string {
  ensureTalonCliOnPath({ pathEnv: process.env.PATH ?? "" });
