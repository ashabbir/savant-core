...
import { handleSessionRegistryHttpRequest } from "./session-registry-http.js";
...
export function createHooksRequestHandler(
  ...
  return async (req, res) => {
    ...
    if (url.searchParams.has("token")) {
      ...
      res.end(
        "Hook token must be provided via Authorization: Bearer <token> or X-Talon-Token header (query parameters are not allowed).",
      );
      return true;
    }
...
export function createGatewayHttpServer(opts: {
  ...
  async function handleRequest(req: IncomingMessage, res: ServerResponse) {
    ...
    try {
      ...
      if (
        await handleAgentRegistryHttpRequest(req, res, {
          auth: resolvedAuth,
          trustedProxies,
        })
      ) {
      ...
      if (
        await handleAuthRegistryHttpRequest(req, res, {
          auth: resolvedAuth,
          trustedProxies,
        })
      ) {
      ...
      if (
        await handleSessionRegistryHttpRequest(req, res, {
          auth: resolvedAuth,
          trustedProxies,
        })
      ) {
