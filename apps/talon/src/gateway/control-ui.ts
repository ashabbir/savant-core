...
function injectControlUiConfig(html: string, opts: ControlUiInjectionOpts): string {
  ...
  const script =
    `<script>` +
    `window.__TALON_CONTROL_UI_BASE_PATH__=${JSON.stringify(basePath)};` +
    `window.__TALON_ASSISTANT_NAME__=${JSON.stringify(
      assistantName ?? DEFAULT_ASSISTANT_IDENTITY.name,
    )};` +
    `window.__TALON_ASSISTANT_AVATAR__=${JSON.stringify(
      assistantAvatar ?? DEFAULT_ASSISTANT_IDENTITY.avatar,
    )};` +
    `</script>`;
  // Check if already injected
  if (html.includes("__TALON_ASSISTANT_NAME__")) {
    return html;
  }
...
export function handleControlUiHttpRequest(
  ...
  const basePath = normalizeControlUiBasePath(opts?.basePath);
  const pathname = url.pathname;
  const isUiPath = basePath
    ? pathname === basePath || pathname.startsWith(`${basePath}/`)
    : pathname === "/ui" || pathname.startsWith("/ui/");

  // Ignore non-UI routes so API handlers can process them.
  if (!isUiPath) {
    return false;
  }

  if (req.method !== "GET" && req.method !== "HEAD") {
    console.log("[debug] control-ui 405:", req.method, req.url);
    res.statusCode = 405;
    res.setHeader("Content-Type", "text/plain; charset=utf-8");
