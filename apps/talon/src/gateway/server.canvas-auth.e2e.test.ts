...
async function withTempConfig(params: { cfg: unknown; run: () => Promise<void> }): Promise<void> {
  const prevConfigPath = process.env.TALON_CONFIG_PATH;
  const prevDisableCache = process.env.TALON_DISABLE_CONFIG_CACHE;

  const dir = await mkdtemp(path.join(os.tmpdir(), "talon-canvas-auth-test-"));
  const configPath = path.join(dir, "talon.json");

  process.env.TALON_CONFIG_PATH = configPath;
  process.env.TALON_DISABLE_CONFIG_CACHE = "1";
  ...
  try {
    ...
    if (prevConfigPath === undefined) {
      ...
      process.env.TALON_CONFIG_PATH = prevConfigPath;
    }
    if (prevDisableCache === undefined) {
      delete process.env.TALON_DISABLE_CONFIG_CACHE;
    } else {
      process.env.TALON_DISABLE_CONFIG_CACHE = prevDisableCache;
    }
