...
import { createTalonTools } from "../agents/talon-tools.js";
...
beforeAll(async () => {
  prevGatewayPort = process.env.TALON_GATEWAY_PORT;
  prevGatewayToken = process.env.TALON_GATEWAY_TOKEN;
  gatewayPort = await getFreePort();
  process.env.TALON_GATEWAY_PORT = String(gatewayPort);
  process.env.TALON_GATEWAY_TOKEN = "test-token";
...
afterAll(async () => {
  ...
  if (prevGatewayPort === undefined) {
    ...
    process.env.TALON_GATEWAY_PORT = prevGatewayPort;
  }
  if (prevGatewayToken === undefined) {
    delete process.env.TALON_GATEWAY_TOKEN;
  } else {
    process.env.TALON_GATEWAY_TOKEN = prevGatewayToken;
  }
});
...
describe("sessions_send gateway loopback", () => {
  it("returns reply when lifecycle ends before agent.wait", async () => {
    ...
    const tool = createTalonTools().find((candidate) => candidate.name === "sessions_send");
    if (!tool) {
