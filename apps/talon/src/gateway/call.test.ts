...
describe("callGateway url override auth requirements", () => {
  ...
  it("throws when url override is set without explicit credentials", async () => {
    process.env.TALON_GATEWAY_TOKEN = "env-token";
    process.env.TALON_GATEWAY_PASSWORD = "env-password";
    loadConfig.mockReturnValue({
      gateway: {
        mode: "local",
...
describe("callGateway password resolution", () => {
  const originalEnvPassword = process.env.TALON_GATEWAY_PASSWORD;
  ...
  afterEach(() => {
    if (originalEnvPassword == null) {
      ...
      process.env.TALON_GATEWAY_PASSWORD = originalEnvPassword;
    }
  });
  ...
  it("prefers env password over local config password", async () => {
    process.env.TALON_GATEWAY_PASSWORD = "from-env";
    loadConfig.mockReturnValue({
      gateway: {
        mode: "local",
        bind: "loopback",
...
describe("callGateway token resolution", () => {
  const originalEnvToken = process.env.TALON_GATEWAY_TOKEN;
  ...
  afterEach(() => {
    if (originalEnvToken == null) {
      ...
      process.env.TALON_GATEWAY_TOKEN = originalEnvToken;
    }
  });
