...
import { DEFAULT_ACCOUNT_ID } from "../agents/session-key.js";
...
const testConfigRoot = {
  value: path.join(os.tmpdir(), `talon-gateway-test-${process.pid}-${crypto.randomUUID()}`),
};

export const setTestConfigRoot = (root: string) => {
  testConfigRoot.value = root;
  process.env.TALON_CONFIG_PATH = path.join(root, "talon.json");
};
...
vi.mock("../config/config.js", async () => {
  ...
  const resolveConfigPath = () => path.join(testConfigRoot.value, "talon.json");
  ...
  return {
    ...
    loadConfig: () => {
      ...
      const defaults = {
        ...
        workspace: path.join(os.tmpdir(), "talon-gateway-test"),
        ...fileDefaults,
...
vi.mock("../plugins/loader.js", async () => {
  ...
  return {
    ...actual,
    loadTalonPlugins: () => pluginRegistryState.registry,
  };
});
