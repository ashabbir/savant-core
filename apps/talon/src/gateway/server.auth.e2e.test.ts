...
describe("gateway server auth/connect", () => {
  describe("default auth (token)", () => {
    ...
    test("closes silent handshakes after timeout", { timeout: 60_000 }, async () => {
      vi.useRealTimers();
      const prevHandshakeTimeout = process.env.TALON_TEST_HANDSHAKE_TIMEOUT_MS;
      process.env.TALON_TEST_HANDSHAKE_TIMEOUT_MS = "50";
      try {
        ...
        if (prevHandshakeTimeout === undefined) {
          delete process.env.TALON_TEST_HANDSHAKE_TIMEOUT_MS;
        } else {
          process.env.TALON_TEST_HANDSHAKE_TIMEOUT_MS = prevHandshakeTimeout;
        }
      }
    });
  ...
  describe("token auth", () => {
    ...
    beforeAll(async () => {
      prevToken = process.env.TALON_GATEWAY_TOKEN;
      process.env.TALON_GATEWAY_TOKEN = "secret";
      port = await getFreePort();
      server = await startGatewayServer(port);
    });
    ...
    afterAll(async () => {
      ...
      if (prevToken === undefined) {
        ...
        process.env.TALON_GATEWAY_TOKEN = prevToken;
      }
    });
  ...
  test("allows control ui with device identity when insecure auth is enabled", async () => {
    ...
    const prevToken = process.env.TALON_GATEWAY_TOKEN;
    process.env.TALON_GATEWAY_TOKEN = "secret";
  ...
  test("requires pairing for scope upgrades", async () => {
    ...
    const { server, ws, port, prevToken } = await startServerWithClient("secret");
    const identityDir = await mkdtemp(join(tmpdir(), "talon-device-scope-"));
