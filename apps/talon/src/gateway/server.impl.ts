...
import { ensureTalonCliOnPath } from "../infra/path-env.js";
...
export async function startGatewayServer(
  ...
  process.env.TALON_GATEWAY_PORT = String(port);
  logAcceptedEnvOption({
    key: "TALON_RAW_STREAM",
    description: "raw stream logging enabled",
  });
  logAcceptedEnvOption({
    key: "TALON_RAW_STREAM_PATH",
  ...
  if (configSnapshot.legacyIssues.length > 0) {
    ...
    if (!migrated) {
      throw new Error(
        `Legacy config entries detected but auto-migration failed. Run "${formatCliCommand("talon doctor")}" to migrate.`,
      );
    }
  ...
  if (configSnapshot.exists && !configSnapshot.valid) {
    ...
    throw new Error(
      `Invalid config at ${configSnapshot.path}.\n${issues}\nRun "${formatCliCommand("talon doctor")}" to repair, then retry.`,
    );
  }
  ...
  const wizardRunner = opts.wizardRunner ?? (async () => { });
  const { wizardSessions, findRunningWizard, purgeWizardSession } = createWizardSessionTracker();

  const deps = createDefaultDeps();
  let canvasHostServer: any = null;
  const gatewayTls = await loadGatewayTlsRuntime(cfgAtStart.gateway?.tls, log.child("tls"));
  if (cfgAtStart.gateway?.tls?.enabled && !gatewayTls.enabled) {
    throw new Error(gatewayTls.error ?? "gateway tls: failed to enable");
  }
  const {
    canvasHost: _unusedCanvasHost,
    httpServer,
    httpServers,
    httpBindHosts,
    wss,
  ...
  void cron.start().catch((err: any) => logCron.error(`failed to start: ${String(err)}`));
  ...
  attachGatewayWsHandlers({
    ...
    canvasHostEnabled: Boolean(_unusedCanvasHost),
