...
beforeAll(async () => {
  previousToken = process.env.TALON_GATEWAY_TOKEN;
...
afterAll(async () => {
  ...
  if (previousToken === undefined) {
    ...
    process.env.TALON_GATEWAY_TOKEN = previousToken;
  }
});
...
describe("gateway server health/presence", () => {
  ...
  test("presence includes client fingerprint", async () => {
    const identityPath = path.join(os.tmpdir(), `talon-device-${randomUUID()}.json`);
