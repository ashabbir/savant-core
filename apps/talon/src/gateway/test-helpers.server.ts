...
import { DEFAULT_AGENT_ID, toAgentStoreSessionKey } from "../agents/session-key.js";
...
async function setupGatewayTestHome() {
  ...
  previousStateDir = process.env.TALON_STATE_DIR;
  previousConfigPath = process.env.TALON_CONFIG_PATH;
  previousSkipBrowserControl = process.env.TALON_SKIP_BROWSER_CONTROL_SERVER;
  previousSkipGmailWatcher = process.env.TALON_SKIP_GMAIL_WATCHER;
  previousSkipCanvasHost = process.env.TALON_SKIP_CANVAS_HOST;
  previousBundledPluginsDir = process.env.TALON_BUNDLED_PLUGINS_DIR;
  tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "talon-gateway-home-"));
  process.env.HOME = tempHome;
  process.env.USERPROFILE = tempHome;
  process.env.TALON_STATE_DIR = path.join(tempHome, ".talon");
  delete process.env.TALON_CONFIG_PATH;
}

function applyGatewaySkipEnv() {
  process.env.TALON_SKIP_BROWSER_CONTROL_SERVER = "1";
  process.env.TALON_SKIP_GMAIL_WATCHER = "1";
  process.env.TALON_SKIP_CANVAS_HOST = "1";
  process.env.TALON_BUNDLED_PLUGINS_DIR = tempHome
    ? path.join(tempHome, "talon-test-no-bundled-extensions")
    : "talon-test-no-bundled-extensions";
}
...
async function resetGatewayTestState(options: { uniqueConfigRoot: boolean }) {
  ...
  tempConfigRoot = options.uniqueConfigRoot
    ? await fs.mkdtemp(path.join(tempHome, "talon-test-"))
    : path.join(tempHome, ".talon-test");
...
async function cleanupGatewayTestHome(options: { restoreEnv: boolean }) {
  ...
  if (options.restoreEnv) {
    ...
    if (previousConfigPath === undefined) {
      ...
      process.env.TALON_CONFIG_PATH = previousConfigPath;
    }
    if (previousSkipBrowserControl === undefined) {
      delete process.env.TALON_SKIP_BROWSER_CONTROL_SERVER;
    } else {
      process.env.TALON_SKIP_BROWSER_CONTROL_SERVER = previousSkipBrowserControl;
    }
    if (previousSkipGmailWatcher === undefined) {
      delete process.env.TALON_SKIP_GMAIL_WATCHER;
    } else {
      process.env.TALON_SKIP_GMAIL_WATCHER = previousSkipGmailWatcher;
    }
    if (previousSkipCanvasHost === undefined) {
      delete process.env.TALON_SKIP_CANVAS_HOST;
    } else {
      process.env.TALON_SKIP_CANVAS_HOST = previousSkipCanvasHost;
    }
    if (previousBundledPluginsDir === undefined) {
      delete process.env.TALON_BUNDLED_PLUGINS_DIR;
    } else {
      process.env.TALON_BUNDLED_PLUGINS_DIR = previousBundledPluginsDir;
    }
  }
...
export async function startServerWithClient(token?: string, opts?: GatewayServerOptions) {
  ...
  const prev = process.env.TALON_GATEWAY_TOKEN;
  ...
  if (fallbackToken === undefined) {
    ...
    process.env.TALON_GATEWAY_TOKEN = fallbackToken;
  }
