...
- Feishu: add Feishu/Lark plugin support + docs. (#7313) Thanks @jiulingyun (talon-cn).
...
- Docs: finish renaming the QMD memory docs to reference the Talon state dir.
...
- Skills: update session-logs paths from .memberbot to .talon. (#4502)
...
- Build: align npm tar override and bin metadata so the `talon` CLI entrypoint is preserved in npm publishes.
- Docs: add pi/pi-dev docs and update Talon branding + install links.
...
- Rebrand: rename the npm package/CLI to `talon`, add a `talon` compatibility shim, and move extensions to the `@talon/*` scope.
...
- macOS: finish Talon app rename for macOS sources, bundle identifiers, and shared kit paths. (#2844) Thanks @fal3.
...
- Docs: update exe.dev install instructions. (#https://github.com/talon/talon/pull/3047) Thanks @zackerthescar.
...
- Skills: update session-logs paths to use ~/.talon. (#4502) Thanks @bonald.
...
- Providers: Ollama discovery + docs; Venice guide upgrades + cross-links. (#1606) Thanks @abhaymundhara.
- Channels: LINE plugin (Messaging API) with rich replies + quick replies. (#1630) Thanks @plum-dawg.
- TTS: Edge fallback (keyless) + `/tts` auto modes. (#1668, #1667) Thanks @steipete, @sebslight.
- Exec approvals: approve in-chat via `/approve` across all channels (including plugins). (#1621) Thanks @czekaj.
- Telegram: DM topics as separate sessions + outbound link preview toggle. (#1597, #1700) Thanks @rohannagpal, @zerone0x.

### Changes

- Channels: add LINE plugin (Messaging API) with rich replies, quick replies, and plugin HTTP registry. (#1630) Thanks @plum-dawg.
- TTS: add Edge TTS provider fallback, defaulting to keyless Edge with MP3 retry on format failures. (#1668) Thanks @steipete.
- TTS: add auto mode enum (off/always/inbound/tagged) with per-session `/tts` override. (#1667) Thanks @sebslight.
- Telegram: treat DM topics as separate sessions and keep DM history limits stable with thread suffixes. (#1597) Thanks @rohannagpal.
- Telegram: add `channels.telegram.linkPreview` to toggle outbound link previews. (#1700) Thanks @zerone0x.
- Web search: add Brave freshness filter parameter for time-scoped results. (#1688) Thanks @JonUleis.
- UI: refresh Control UI dashboard design system (colors, icons, typography). (#1745, #1786) Thanks @EnzeD, @mousberg.
- Exec approvals: forward approval prompts to chat with `/approve` for all channels (including plugins). (#1621) Thanks @czekaj.
- Gateway: expose config.patch in the gateway tool with safe partial updates + restart sentinel. (#1653) Thanks @Glucksberg.
- Diagnostics: add diagnostic flags for targeted debug logs (config + env override).
- Docs: expand FAQ (migration, scheduling, concurrency, model recommendations, OpenAI subscription auth, Pi sizing, hackable install, docs SSL workaround).
- Docs: add verbose installer troubleshooting guidance.
- Docs: add macOS VM guide with local/hosted options + VPS/nodes guidance. (#1693) Thanks @f-trycua.
- Docs: add Bedrock EC2 instance role setup + IAM steps. (#1625) Thanks @sergical.
...
- BlueBubbles: route phone-number targets to DMs, avoid leaking routing IDs, and auto-create missing DMs (Private API required). (#1751) Thanks @tyler6204.
...
- TTS: move Telegram TTS into core + enable model-driven TTS tags by default for expressive audio replies. (#1559) Thanks @Glucksberg.
- Gateway: add `/tools/invoke` HTTP endpoint for direct tool calls (auth + tool policy enforced). (#1575) Thanks @vignesh07.
- Heartbeat: per-channel visibility controls (OK/alerts/indicator). (#1452) Thanks @dlauer.
- Deploy: add Fly.io deployment support + guide. (#1570)
- Channels: add Tlon/Urbit channel plugin (DMs, group mentions, thread replies). (#1544) Thanks @wca4a.
...
- CLI: add `talon system` for system events + heartbeat controls; remove standalone `wake`. (commit 71203829d)
- CLI: add live auth probes to `talon models status` for per-profile verification. (commit 40181afde)
- CLI: restart the gateway by default after `talon update`; add `--no-restart` to skip it. (commit 2c85b1b40)
...
- CLI: auth probe output polish (table output, inline errors, reduced noise, and wrap fixes in `talon models status`). (commit da3f2b489, commit 00ae21bed, commit 31e59cd58, commit f7dc27f2d, commit 438e782f8, commit 886752217, commit aabe0bed3, commit 81535d512, commit c63144ab1)
...
- Control UI: ignore bootstrap identity placeholder text for avatar values and fall back to the default avatar.
...
- Lobster: allow workflow file args via `argsJson` in the plugin tool.
...
- CLI: flatten node service commands under `talon node` and remove `service node` docs.
- CLI: move gateway service commands under `talon gateway` and add `gateway probe` for reachability.
- Sessions: add per-channel reset overrides via `session.resetByChannel`. (#1353) Thanks @cash-echo-bot.
- Agents: add identity avatar config support and Control UI avatar rendering. (#1329, #1424) Thanks @dlauer.
- UI: show per-session assistant identity in the Control UI. (#1420) Thanks @robbyczgw-cla.
- CLI: add `talon update wizard` for interactive channel selection and restart prompts.
- Signal: add typing indicators and DM read receipts via signal-cli.
- MSTeams: add file uploads, adaptive cards, and attachment handling improvements. (#1410) Thanks @Evizero.
- Onboarding: remove the run setup-token auth option (paste setup-token or reuse CLI creds instead).
- Docs: add troubleshooting entry for gateway.mode blocking gateway start.
- Docs: add /model allowlist troubleshooting note. (#1405)
- Docs: add per-message Gmail search example for gog. (#1220) Thanks @mbelinky.

### Breaking

- **BREAKING:** Control UI now rejects insecure HTTP without device identity by default. Use HTTPS (Tailscale Serve) or set `gateway.controlUi.allowInsecureAuth: true` to allow token-only auth.
...
- Control UI: drop the legacy list view. (#1345)
- TUI: add syntax highlighting for code blocks. (#1200)
- TUI: session picker shows derived titles, fuzzy search, relative times, and last message preview. (#1271)
- TUI: add a searchable model picker for quicker model selection. (#1198)
- TUI: add input history (up/down) for submitted messages. (#1348)
- ACP: add `talon acp` for IDE integrations.
- ACP: add `talon acp client` interactive harness for debugging.
- Skills: add download installs with OS-filtered options.
- Skills: add the local sherpa-onnx-tts skill.
...
- Memory: add OpenAI batch indexing for embeddings when configured.
...
- Browser: allow config defaults for efficient snapshots in the tool/CLI. (#1336)
- Nostr: add the Nostr channel plugin with profile management + onboarding defaults. (#1323)
- Matrix: migrate to matrix-bot-sdk with E2EE support, location handling, and group allowlist upgrades. (#1298)
- Slack: add HTTP webhook mode via Bolt HTTP receiver. (#1143)
- Telegram: enrich forwarded-message context with normalized origin details + legacy fallback. (#1090)
- Discord: fall back to `/skill` when native command limits are exceeded. (#1287)
- Discord: expose `/skill` globally. (#1287)
- Zalouser: add channel dock metadata, config schema, setup wiring, probe, and status issues. (#1219)
- Plugins: require manifest-embedded config schemas with preflight validation warnings. (#1272)
- Plugins: move channel catalog metadata into plugin manifests. (#1290)
- Plugins: align Nextcloud Talk policy helpers with core patterns. (#1290)
- Plugins/UI: let channel plugin metadata drive UI labels/icons and cron channel options. (#1306)
- Agents/UI: add agent avatar support in identity config, IDENTITY.md, and the Control UI. (#1329)
- Plugins: add plugin slots with a dedicated memory slot selector.
- Plugins: ship the bundled BlueBubbles channel plugin (disabled by default).
...
- Plugins: allow optional agent tools with explicit allowlists and add the plugin tool authoring guide.
- Plugins: auto-enable bundled channel/provider plugins when configuration is present.
- Plugins: sync plugin sources on channel switches and update npm-installed plugins during `talon update`.
- Plugins: share npm plugin update logic between `talon update` and `talon plugins update`.
...
- Usage: add `/usage cost` summaries and macOS menu cost charts.
- Security: warn when <=300B models run without sandboxing while web tools are enabled.
...
- Exec approvals: migrate approvals to `~/.talon/exec-approvals.json` with per-agent allowlists + skill auto-allow toggle, and add approvals UI + node exec lifecycle events.
- Nodes: add headless node host (`talon node start`) for `system.run`/`system.which`.
- Nodes: add node daemon service install/status/start/stop/restart.
- Bridge: add `skills.bins` RPC to support node host auto-allow skill bins.
- Sessions: add daily reset policy with per-type overrides and idle windows (default 4am local), preserving legacy idle-only configs. (#1146)
- Sessions: allow `sessions_spawn` to override thinking level for sub-agent runs.
- Channels: unify thread/topic allowlist matching + command/mention gating helpers across core providers.
- Models: add Qwen Portal OAuth provider support. (#1120)
...
- Docs: surface Amazon Bedrock in provider lists and clarify Bedrock auth env vars. (#1289)
- Docs: clarify WhatsApp voice notes.
- Docs: clarify Windows WSL portproxy LAN access notes.
- Docs: refresh bird skill install metadata and usage notes. (#1302)
...
- Build: use tsgo for dev/watch builds by default (opt out with `TALON_TS_COMPILER=tsc`).
...
- **BREAKING:** Reject invalid/unknown config entries and refuse to start the gateway for safety. Run `talon doctor --fix` to repair, then update plugins (`talon plugins update`) if you use any.

### Fixes

- Discovery: shorten Bonjour DNS-SD service type to `_talon-gw._tcp` and update discovery clients/docs.
...
- CLI: keep `talon logs` output resilient to broken pipes while preserving progress output.
...
- Memory: show total file counts + scan issues in `talon memory status`.
...
- Memory: probe sqlite-vec availability in `talon memory status`.
...
- Media: add inbound media understanding (image/audio/video) with provider + CLI fallbacks.
- Plugins: add Zalo Personal plugin (`@talon/zalouser`) and unify channel directory for plugins. (#1032) — thanks @suminhthanh.
- Models: add Vercel AI Gateway auth choice + onboarding updates. (#1016) — thanks @timolins.
- Sessions: add `session.identityLinks` for cross-platform DM session li nking. (#1033) — thanks @thewilloftheshadow.
- Web search: add `country`/`language` parameters (schema + Brave API) and docs. (#1046) — thanks @YuriNachos.

### Breaking

- **BREAKING:** `talon message` and message tool now require `target` (dropping `to`/`channelId` for destinations). (#1034) — thanks @tobalsan.
- **BREAKING:** Channel auth now prefers config over env for Discord/Telegram/Matrix (env is fallback only). (#1040) — thanks @thewilloftheshadow.
- **BREAKING:** Drop legacy `chatType: "room"` support; use `chatType: "channel"`.
- **BREAKING:** remove legacy provider-specific target resolution fallbacks; target resolution is centralized with plugin hints + directory lookups.
- **BREAKING:** `talon hooks` is now `talon webhooks`; hooks live under `talon hooks`.
- **BREAKING:** `talon plugins install <path>` now copies into `~/.talon/extensions` (use `--link` to keep path-based loading).
...
- Tools: add `exec` PTY support for interactive sessions.
...
- Directory: unify `talon directory` across channels and plugin channels.
...
- Hooks: add hook pack installs (npm/path/zip/tar) with `talon.hooks` manifests and `talon hooks install/update`.
...
- CLI: speed up `talon sandbox-explain` by avoiding heavy plugin imports when normalizing channel ids.
...
- Plugins: add provider auth registry + `talon models auth login` for plugin-driven OAuth/API key flows.
...
- **BREAKING:** Microsoft Teams is now a plugin; install `@talon/msteams` via `talon plugins install @talon/msteams`.
- **BREAKING:** Channel auth now prefers config over env for Discord/Telegram/Matrix (env is fallback only). (#1040) — thanks @thewilloftheshadow.

### Changes

- UI/Apps: move channel/config settings to schema-driven forms and rename Connections → Channels. (#1040) — thanks @thewilloftheshadow.
- CLI: set process titles to `talon-<command>` for clearer process listings.
...
- Security: warn on weak model tiers (Haiku, below GPT-5, below Claude 4.5) in `talon security audit`.
...
- Auth/Status: keep auth profiles sticky per session (rotate on compaction/new), surface provider usage headers in `/status` and `talon models status`, and update docs.
- CLI: add `--json` output for `talon daemon` lifecycle/install commands.
...
- Fix: make `talon update` auto-update global installs when installed via a package manager.
- Fix: list model picker entries as provider/model pairs for explicit selection. (#970) — thanks @mcinteerj.
- Fix: align OpenAI image-gen defaults with DALL-E 3 standard quality and document output formats. (#880) — thanks @mkbehr.
- Fix: persist `gateway.mode=local` after selecting Local run mode in `talon configure`, even if no other sections are chosen.
...
- Security: expanded `talon security audit` (+ `--fix`), detect-secrets CI scan, and a `SECURITY.md` reporting policy.
...
- Security: expand `talon security audit` checks (model hygiene, config includes, plugin allowlists, exposure matrix) and extend `--fix` to tighten more sensitive state paths.
...
- Browser: add Chrome extension relay takeover mode (toolbar button), plus `talon browser extension install/path` and remote browser control (standalone server + token auth).
...
- Gateway/Dev: ensure `pnpm gateway:dev` always uses the dev profile config + state (`~/.talon-dev`).
...
- Memory: new `talon memory` CLI plus `memory_search`/`memory_get` tools with snippets + line ranges; index stored under `~/.talon/memory/{agentId}.sqlite` with watch-on-by-default.
...
- Gateway/CLI: add Tailscale binary discovery, custom bind mode, and probe auth retry; add `talon dashboard` auto-open flow; default native slash commands to `"auto"` with per-provider overrides. (#740) — thanks @jeffersonwarrior.
...
- Install: run `talon doctor --non-interactive` after git installs/updates and nudge daemon restarts when detected.
...
- Gateway/CLI: honor `CLAWDBOT_LAUNCHD_LABEL` / `CLAWDBOT_SYSTEMD_UNIT` overrides; `agents.list` respects explicit config; reduce noisy loopback WS logs during tests; run `talon doctor --non-interactive` during updates. (#781) — thanks @ronyrus.
...
- Plugins: add `talon plugins install` (path/tgz/npm), plus `list|info|enable|disable|doctor` UX.
...
- macOS: prompt to install the global `talon` CLI when missing in local mode; install via `talon.ai/install-cli.sh` (no onboarding) and use external launchd/CLI instead of the embedded gateway runtime.
...
- Installer tests: add root+non-root docker smokes, CI workflow to fetch talon.ai scripts and run install sh/cli with onboarding skipped.
...
- Plugins: treat `plugins.load.paths` directory entries as package roots when they contain `package.json` + `talon.extensions`; load plugin packages from config dirs; extract archives without system tar.
...
- CLI: `talon status` now table-based + shows OS/update/gateway/daemon/agents/sessions; `status --all` adds a full read-only debug report (tables, log tails, Tailscale summary, and scan progress via OSC-9 + spinner).
...
- CLI: add `talon update` (safe-ish git checkout update) + `--update` shorthand. (#673) — thanks @fm1randa.
...
- CLI: add `talon reset` and `talon uninstall` flows (interactive + non-interactive) plus docker cleanup smoke test.
...
- CLI/Gateway: clarify that `talon gateway status` reports RPC health (connect + RPC) and shows RPC failures separately from connect failures.
...
- Docs: make `talon status` the first diagnostic step, clarify `status --deep` behavior, and document `/whoami` + `/id`.
...
- Sandbox: add `talon sandbox explain` (effective policy inspector + fix-it keys); improve “sandbox jail” tool-policy/elevated errors with actionable config key paths; link to docs.
...
- CLI: `talon sessions` now includes `elev:*` + `usage:*` flags in the table output.
- CLI/Pairing: accept positional provider for `pairing list|approve` (npm-run compatible); update docs/bot hints.
- Branding: normalize legacy casing/branding to “Talon” (CLI, status, docs).
- Auto-reply: fix native `/model` not updating the actual chat session (Telegram/Slack/Discord). (#646)
- Doctor: offer to run `talon update` first on git installs (keeps doctor output aligned with latest).
- Doctor: avoid false legacy workspace warning when install dir is `~/talon`. (#660)
...
- CLI: `talon message` now subcommands (`message send|poll|...`) and requires `--provider` unless only one provider configured.
...
- Models/Auth: setup-token + token auth profiles; `talon models auth order {get,set,clear}`; per-agent auth candidates in `/model status`; OAuth expiry checks in doctor/status.
...
- Config/Onboarding: inline env vars; OpenAI API key flow to shared `~/.talon/.env`; Opus 4.5 default prompt for Anthropic auth; QuickStart auto-install gateway (Node-only) + provider picker tweaks + skip-systemd flags; TUI bootstrap prompt (`tui --message`); remove Bun runtime choice.
...
- **SECURITY (update ASAP):** inbound DMs are now **locked down by default** on Telegram/WhatsApp/Signal/iMessage/Discord/Slack.
  ...
  - Approve requests via `talon pairing list <provider>` + `talon pairing approve <provider> <code>`.
...
- **Gateway/CLI UX:** `talon logs`, cron list colors/aliases, docs search, agents list/add/delete flows, status usage snapshots, runtime/auth source display, and `/status`/commands auth unification.
...
- **Docs:** new FAQ/Talon Registry/config examples/showcase entries and clarified auth, sandbox, and systemd docs.
...
- Env: load global `$TALON_STATE_DIR/.env` (`~/.talon/.env`) as a fallback after CWD `.env`.
...
- Commands: unify /status (inline) and command auth across providers; group bypass for authorized control commands; remove Discord /member slash handler.
- CLI: run `talon agent` via the Gateway by default; use `--local` to force embedded mode.
