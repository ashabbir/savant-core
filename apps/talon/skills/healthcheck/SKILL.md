---
name: healthcheck
description: Host security hardening and risk-tolerance configuration for Talon deployments. Use when a user asks for security audits, firewall/SSH/update hardening, risk posture, exposure review, Talon cron scheduling for periodic checks, or version status checks on a machine running Talon (laptop, workstation, Pi, VPS).
---

# Talon Host Hardening

## Overview

Assess and harden the host running Talon, then align it to a user-defined risk tolerance without breaking access. Use Talon security tooling as a first-class signal, but treat OS hardening as a separate, explicit set of steps.
...
- Never claim Talon changes the host firewall, SSH, or OS updates; it does not.
...
5. Talon gateway status and bind address.
...
9. OS automatic security updates status.
   Note: these are not blocking items, but are highly recommended, especially if Talon can access sensitive data.
...
### 2) Run Talon security audits (read-only)

As part of the default read-only checks, run `talon security audit --deep`. Only offer alternatives if the user requests them:

1. `talon security audit` (faster, non-probing)
2. `talon security audit --json` (structured output)

Offer to apply Talon safe defaults (numbered):

1. `talon security audit --fix`

Be explicit that `--fix` only tightens Talon defaults and file permissions. It does not change host firewall, SSH, or OS update policies.
...
### 3) Check Talon version/update status (read-only)

As part of the default read-only checks, run `talon update status`.
...
- Credential hygiene notes (location of Talon creds, prefer disk encryption)
...
After Talon install or first hardening pass, run at least one baseline audit and version check:

- `talon security audit`
- `talon security audit --deep`
- `talon update status`

Ongoing monitoring is recommended. Use the Talon cron tool/CLI to schedule periodic audits (Gateway scheduler). Do not create scheduled tasks without explicit approval. Store outputs in a user-approved location and avoid secrets in logs.
...
1. “Do you want me to schedule periodic audits (e.g., daily/weekly) via `talon cron add`?”

If the user says yes, ask for:

- cadence (daily/weekly), preferred time window, and output location
- whether to also schedule `talon update status`
...
Before creating, `talon cron list` and match on exact `name`. If found, `talon cron edit <id> ...`.
If not found, `talon cron add --name <name> ...`.

Also offer a periodic version check so the user can decide when to update (numbered):

1. `talon update status` (preferred for source checkouts and channels)
2. `npm view talon version` (published npm version)

## Talon command accuracy

Use only supported commands and flags:

- `talon security audit [--deep] [--fix] [--json]`
- `talon status` / `talon status --deep`
- `talon health --json`
- `talon update status`
- `talon cron add|list|runs|run`

Do not invent CLI flags or imply Talon enforces host firewall/SSH policies.
...
Follow the durable-memory prompt format used by Talon compaction:
