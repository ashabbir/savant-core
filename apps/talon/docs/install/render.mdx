...
Deploy Talon on Render using Infrastructure as Code. The included `render.yaml` Blueprint defines your entire stack declaratively, service, disk, environment variables, so you can deploy with a single click and version your infrastructure alongside your code.
...
[Deploy to Render](https://render.com/deploy?repo=https://github.com/talon/talon)
...
repository configures everything needed to run Talon:
...
services:
  - type: web
    ...
    envVars:
      ...
      - key: TALON_STATE_DIR
        value: /data/.talon
      - key: TALON_WORKSPACE_DIR
        value: /data/workspace
      - key: TALON_GATEWAY_TOKEN
...
The web dashboard is available at `https://<your-service>.onrender.com/talon`.
...
If you use the original Talon repository, Render will not auto-deploy your Talon. To update it, run a manual Blueprint sync from the dashboard.
...
For Talon, vertical scaling is usually sufficient. Horizontal scaling requires sticky sessions or external state management.
...
This downloads a portable backup you can restore on any Talon host.
