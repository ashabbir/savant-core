...
Deploy Talon on Railway with a one-click template and finish setup in your browser.
...
- `https://<your-railway-domain>/talon` — Control UI
...
- `TALON_STATE_DIR=/data/.talon` (recommended)
- `TALON_WORKSPACE_DIR=/data/workspace` (recommended)
- `TALON_GATEWAY_TOKEN` (recommended; treat as an admin secret)
...
This exports your Talon state + workspace so you can migrate to another host without losing config or memory.
