...
Tlon is a decentralized messenger built on Urbit. Talon connects to your Urbit ship and can
...
```bash
talon plugins install @talon/tlon
```

Local checkout (when running from a git repo):

```bash
talon plugins install ./extensions/tlon
```
...
Use these with `talon message send` or cron delivery:
...
- Thread replies: if the inbound message is in a thread, Talon replies in-thread.
