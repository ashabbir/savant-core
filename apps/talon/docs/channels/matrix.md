...
Matrix is an open, decentralized messaging protocol. Talon connects as a Matrix **user**
...
```bash
talon plugins install @talon/matrix
```

Local checkout (when running from a git repo):

```bash
talon plugins install ./extensions/matrix
```
...
1. Install the Matrix plugin:
   - From npm: `talon plugins install @talon/matrix`
   - From a local checkout: `talon plugins install ./extensions/matrix`
...
3. Get an access token for the bot account:
   ...
   - Or set `channels.matrix.userId` + `channels.matrix.password`: Talon calls the same
     login endpoint, stores the access token in `~/.talon/credentials/matrix/credentials.json`,
...
- On first connection, Talon requests device verification from your other sessions.
- Verify the device in another Matrix client (Element, etc.) to enable key sharing.
- If the crypto module cannot be loaded, E2EE is disabled and encrypted rooms will not decrypt;
  Talon logs a warning.
...
`~/.talon/matrix/accounts/<account>/<homeserver>__<user>/<token-hash>/crypto/`
...
- Approve via:
  - `talon pairing list matrix`
  - `talon pairing approve matrix <CODE>`
...
- On startup, Talon resolves room/user names in allowlists to IDs and logs the mapping; unresolved entries are ignored for allowlist matching.
