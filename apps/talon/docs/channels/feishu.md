...
Feishu (Lark) is a team chat platform used by companies for messaging and collaboration. This plugin connects Talon to a Feishu/Lark bot using the platform’s WebSocket event subscription so messages can be received without exposing a public webhook URL.
...
```bash
talon plugins install @talon/feishu
```

Local checkout (when running from a git repo):

```bash
talon plugins install ./extensions/feishu
```
...
If you just installed Talon, run the wizard:
...
2. Configuring app credentials in Talon
...
1. You already ran `talon channels add` for Feishu
2. The gateway is running (`talon gateway status`)
...
```bash
talon pairing approve feishu <CODE>
```
...
2. Run `talon logs --follow` and look for `chat_id`
...
2. Run `talon logs --follow` and look for `open_id`
...
| Command                    | Description                   |
| -------------------------- | ----------------------------- |
| `talon gateway status`  | Show gateway status           |
| `talon gateway install` | Install/start gateway service |
| `talon gateway stop`    | Stop gateway service          |
| `talon gateway restart` | Restart gateway service       |
| `talon logs --follow`   | Tail gateway logs             |
...
4. Check logs: `talon logs --follow`
...
5. Ensure the gateway is running: `talon gateway status`
6. Check logs: `talon logs --follow`
  ...
  agents: {
    list: [
      { id: "main" },
      {
        ...
        agentDir: "/home/user/.talon/agents/clawd-fan/agent",
      },
      {
        id: "clawd-xi",
        workspace: "/home/user/clawd-xi",
        agentDir: "/home/user/.talon/agents/clawd-xi/agent",
      },
    ],
  },
  bindings: [
