...
read_when:
  - Setting up Twitch chat integration for Talon
title: "Twitch"
---

# Twitch (plugin)

Twitch chat support via IRC connection. Talon connects as a Twitch user (bot account) to receive and send messages in channels.
...
```bash
talon plugins install @talon/twitch
```

Local checkout (when running from a git repo):

```bash
talon plugins install ./extensions/twitch
```
...
4. Configure the token:
   - Env: `TALON_TWITCH_ACCESS_TOKEN=...` (default account only)
...
```json5
{
  channels: {
    twitch: {
      enabled: true,
      username: "talon", // Bot's Twitch account
      accessToken: "oauth:abc123...", // OAuth Access Token (or use TALON_TWITCH_ACCESS_TOKEN env var)
...
```bash
TALON_TWITCH_ACCESS_TOKEN=oauth:abc123...
```
