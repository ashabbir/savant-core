...
read_when:
  - You want Talon to receive DMs via Nostr
...
Nostr is a decentralized protocol for social networking. This channel enables Talon to receive and respond to encrypted direct messages (DMs) via NIP-04.

## Install (on demand)

### Onboarding (recommended)

- The onboarding wizard (`talon onboard`) and `talon channels add` list optional channel plugins.
...
```bash
talon plugins install @talon/nostr
```

Use a local checkout (dev workflows):

```bash
talon plugins install --link <path-to-talon>/extensions/nostr
```
  ...
  "channels": {
    "nostr": {
      ...
      "profile": {
        ...
        "nip05": "talon@example.com",
        "lud16": "talon@example.com"
      }
    }
  }
}
