...
```bash
talon plugins install @talon/nextcloud-talk
```

Local checkout (when running from a git repo):

```bash
talon plugins install ./extensions/nextcloud-talk
```
...
2. On your Nextcloud server, create a bot:
   ...
   ```bash
   ./occ talk:bot:install "Talon" "<shared-secret>" "<webhook-url>" --feature reaction
   ```
...
- Approve via:
  - `talon pairing list nextcloud-talk`
  - `talon pairing approve nextcloud-talk <CODE>`
