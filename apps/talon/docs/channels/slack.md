...
3. Set tokens for Talon and start the gateway.
...
7. Slash Commands → create `/talon` if you use `channels.slack.slashCommand`. If you enable native commands, add one slash command per built-in command (same names as `/help`). Native defaults to off for Slack unless you set `channels.slack.commands.native: true` (global `commands.native` is `"auto"` which leaves Slack off).
...
Talon can use a Slack user token (`xoxp-...`) for read operations (history,
...
- Write operations (send/edit/delete messages, add/remove reactions, pin/unpin,
  ...
  no bot token is available, Talon falls back to the user token.
...
```json
{
  "display_information": {
    "name": "Talon",
    "description": "Slack connector for Talon"
  },
  "features": {
    "bot_user": {
      "display_name": "Talon",
    ...
    "slash_commands": [
      {
        "command": "/talon",
        "description": "Send a message to Talon",
        "should_escape": false
      }
    ]
  },
...
By default, Talon replies in the main channel. Use `channels.slack.replyToMode` to control automatic threading:
...
- If Slack doesn’t provide `channel_type`, Talon infers it from the channel ID prefix (`D`, `C`, `G`) and defaults to `channel` to keep session keys stable.
...
- Approve via: `talon pairing approve slack <code>`.
...
- On startup, Talon resolves channel/user names in allowlists to IDs (when tokens allow)
