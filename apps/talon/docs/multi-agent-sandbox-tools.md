...
For debugging “why is this blocked?”, see [Sandbox vs Tool Policy vs Elevated](/gateway/sandbox-vs-tool-policy-vs-elevated) and `talon sandbox explain`.
...
Legacy `agent.*` configs are migrated by `talon doctor`; prefer `agents.defaults` + `agents.list` going forward.
...
2. **Verify sandbox containers:**
   ...
   ```exec
   docker ps --filter "name=talon-sbx-"
   ```
...
4. **Monitor logs:**
   ...
   ```exec
   tail -f "${TALON_STATE_DIR:-$HOME/.talon}/logs/gateway.log" | grep -E "routing|sandbox|tools"
   ```
