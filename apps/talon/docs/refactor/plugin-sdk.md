...
- Publish as `talon/plugin-sdk` (or export from core under `talon/plugin-sdk`).
- Semver with explicit stability guarantees.

### 2) Plugin Runtime (execution surface, injected)

Scope: everything that touches core runtime behavior.
Accessed via `TalonPluginApi.runtime` so plugins never import `src/**`.
...
export type PluginRuntime = {
  channel: {
    text: {
      ...
      resolveTextChunkLimit(cfg: TalonConfig, channel: string, accountId?: string): number;
      hasControlCommand(text: string, cfg: TalonConfig): boolean;
    };
    reply: {
    ...
    mentions: {
      buildMentionRegexes(cfg: TalonConfig, agentId?: string): RegExp[];
    ...
    debounce: {
      ...
      resolveInboundDebounceMs(cfg: TalonConfig, channel: string): number;
    };
    commands: {
  ...
  state: {
    resolveStateDir(cfg: TalonConfig): string;
  };
};
```
...
- Introduce `talon/plugin-sdk`.
- Add `api.runtime` to `TalonPluginApi` with the surface above.
...
- Plugins declare a required runtime range (e.g., `talonRuntime: ">=2026.2.0"`).
