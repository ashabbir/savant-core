...
- **Ask default:** `on-miss`.
- **Approvals store:** `~/.talon/exec-approvals.json` (JSON, no legacy migration).
- **Runner:** headless system service; UI app hosts a Unix socket for approvals.
- **Node identity:** use existing `nodeId`.
- **Socket auth:** Unix socket + token (cross-platform); split later if needed.
- **Node host state:** `~/.talon/node.json` (node id + pairing token).
...
Path: `~/.talon/exec-approvals.json`
...
```json
{
  "version": 1,
  "socket": {
    "path": "~/.talon/exec-approvals.sock",
...
- Unix socket at `~/.talon/exec-approvals.sock` (0600).
