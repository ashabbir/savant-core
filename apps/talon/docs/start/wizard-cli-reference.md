...
read_when:
  - You need detailed behavior for talon onboard
...
This page is the full reference for `talon onboard`.
...
<Steps>
  <Step title="Existing config detection">
    - If `~/.talon/talon.json` exists, choose Keep, Modify, or Reset.
    - Re-running the wizard does not wipe anything unless you explicitly choose Reset (or pass `--reset`).
    - If config is invalid or contains legacy keys, the wizard stops and asks you to run `talon doctor` before continuing.
  ...
  <Step title="Channels">
    ...
    - DM security: default is pairing. First DM sends a code; approve via
      `talon pairing approve <channel> <code>` or use allowlists.
  </Step>
  ...
  <Step title="Health check">
    - Starts gateway (if needed) and runs `talon health`.
    - `talon status --deep` adds gateway health probes to status output.
  </Step>
  <Step title="Skills">
...
<AccordionGroup>
  ...
  <Accordion title="OpenAI API key">
    ...
    `~/.talon/.env` so launchd can read it.
...
- OAuth credentials: `~/.talon/credentials/oauth.json`
- Auth profiles (API keys + OAuth): `~/.talon/agents/<agentId>/agent/auth-profiles.json`
...
- Command reference: [`talon onboard`](/cli/onboard)
