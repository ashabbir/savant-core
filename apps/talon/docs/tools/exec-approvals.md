...
- **gateway host** → `talon` process on the gateway machine
...
`~/.talon/exec-approvals.json` directly.

CLI: `talon approvals` supports gateway or node editing (see [Approvals CLI](/cli/approvals)).
