---
title: Lobster
summary: "Typed workflow runtime for Talon with resumable approval gates."
description: Typed workflow runtime for Talon — composable pipelines with approval gates.
read_when:
...
Lobster is a workflow shell that lets Talon run multi-step tool sequences as a single, deterministic operation with explicit approval checkpoints.
...
- **One call instead of many**: Talon runs one Lobster tool call and gets a structured result.
...
Talon launches the local `lobster` CLI in **tool mode** and parses a JSON envelope from stdout.
...
gog.gmail.search --query 'newer_than:1d' \
  | talon.invoke --tool message --action send --each --item-key message --args-json '{"provider":"telegram","to":"..."}'
```
...
```lobster
talon.invoke --tool llm-task --action json --args-json '{
...
Lobster can run YAML/JSON workflow files with `name`, `args`, `steps`, `env`, `condition`, and `approval` fields. In Talon tool calls, set `pipeline` to the file path.
...
Install the Lobster CLI on the **same host** that runs the Talon Gateway (see the [Lobster repo](https://github.com/talon/lobster)), and ensure `lobster` is on `PATH`.
...
plugin tools (like `lobster`), Talon keeps core tools enabled. To restrict core
...
→ talon calls gmail.list
→ LLM summarizes
→ User: "draft replies to #2 and #5"
→ LLM drafts
→ User: "send #2"
→ talon calls gmail.send
...
- **No secrets** — Lobster doesn't manage OAuth; it calls Talon tools that do.
