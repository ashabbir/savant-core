---
summary: "Chrome extension: let Talon drive your existing Chrome tab"
read_when:
...
The Talon Chrome extension lets the agent control your **existing Chrome tabs** (your normal Chrome window) instead of launching a separate talon-managed Chrome profile.
...
Talon then controls the attached tab through the normal `browser` tool surface (selecting the right profile).
...
The extension ships inside the Talon release (npm package) as static files. There is no separate “build” step.

After upgrading Talon:

- Re-run `talon browser extension install` to refresh the installed files under your Talon state directory.
- Chrome → `chrome://extensions` → click “Reload” on the extension.

## Use it (no extra config)

Talon ships with a built-in browser profile named `chrome` that targets the extension relay on the default port.
...
- Open the tab you want Talon to control.
...
- `ON`: attached; Talon can drive that tab.
...
Debugging: `talon sandbox explain`
...
`talon browser extension path` prints the **installed** on-disk directory containing the extension files.

The CLI intentionally does **not** print a `node_modules` path. Always run `talon browser extension install` first to copy the extension to a stable location under your Talon state directory.
...
- **This is not isolated** like the dedicated talon-managed profile.
