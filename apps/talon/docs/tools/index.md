---
summary: "Agent tool surface for Talon (browser, canvas, nodes, message, cron) replacing legacy `talon-*` skills"
read_when:
  - Adding or modifying agent tools
  - Retiring or changing `talon-*` skills
title: "Tools"
---

# Tools (Talon)

Talon exposes **first-class agent tools** for browser, canvas, nodes, and cron.
These replace the old `talon-*` skills: the tools are typed, no shelling,
and the agent should rely on them directly.

## Disabling tools

You can globally allow/deny tools via `tools.allow` / `tools.deny` in `talon.json`
...
- If `tools.allow` only references unknown or unloaded plugin tool names, Talon logs a warning and ignores the allowlist so core tools stay available.
...
- `host=node` can target a macOS companion app or a headless node host (`talon node run`).
...
- Requires a Brave API key (recommended: `talon configure --section web`, or set `BRAVE_API_KEY`).
...
Control the dedicated Talon-managed browser.
...
- Quick smoke: `talon nodes canvas a2ui push --node <id> --text "Hello from A2UI"`.
...
- `restart` (authorizes + sends `SIGUSR1` for in-process restart; `talon gateway` restart in-place)
