...
```bash
talon tui --url ws://<host>:<port> --token <gateway-token>
```
...
- Turn delivery on:
  ...
  - or start with `talon tui --deliver`
...
- Check the Gateway logs: `talon logs --follow`.
- Confirm the agent can run: `talon status` and `talon models status`.
...
- No agents in picker: check `talon agents list` and your routing config.
