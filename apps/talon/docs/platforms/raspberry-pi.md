---
summary: "Talon on Raspberry Pi (budget self-hosted setup)"
read_when:
  - Setting up Talon on a Raspberry Pi
  - Running Talon on ARM devices
  - Building a cheap always-on personal AI
title: "Raspberry Pi"
---

# Talon on Raspberry Pi

## Goal

Run a persistent, always-on Talon Gateway on a Raspberry Pi for **~$35-80** one-time cost (no monthly fees).
...
Most Talon features work on ARM64, but some external binaries may need ARM builds:
...
sudo systemctl is-enabled talon
...
```bash
# Check logs
journalctl -u talon --no-pager -n 100

# Common fix: rebuild
cd ~/talon  # if using hackable install
npm run build
sudo systemctl restart talon
```
