...
service via the external `talon` CLI (no embedded runtime). This gives you
...
- The app installs a per‑user LaunchAgent labeled `bot.molt.gateway`
  (or `bot.molt.<profile>` when using `--profile`/`TALON_PROFILE`; legacy `com.talon.*` is supported).
