---
summary: "Use Venice AI privacy-focused models in Talon"
read_when:
  - You want privacy-focused inference in Talon
...
```bash
talon onboard --auth-choice venice-api-key
```
...
```bash
talon chat --model venice/llama-3.3-70b "Hello, are you working?"
```

## Model Selection

After setup, Talon shows all available Venice models. Pick based on your needs:
...
```bash
talon models set venice/claude-opus-45
talon models set venice/llama-3.3-70b
```

List all available models:

```bash
talon models list | grep venice
```

## Configure via `talon configure`

1. Run `talon configure`
2. Select **Model/auth**
3. Choose **Venice AI**
...
Talon automatically discovers models from the Venice API when `VENICE_API_KEY` is set. If the API is unreachable, it falls back to a static catalog.
...
talon chat --model venice/llama-3.3-70b

# Use Claude via Venice (anonymized)
talon chat --model venice/claude-opus-45

# Use uncensored model
talon chat --model venice/venice-uncensored

# Use vision model with image
talon chat --model venice/qwen3-vl-235b-a22b

# Use coding model
talon chat --model venice/qwen3-coder-480b-a35b-instruct
```
...
The Venice model catalog updates dynamically. Run `talon models list` to see currently available models. Some models may be temporarily offline.
