---
summary: "Use MiniMax M2.1 in Talon"
read_when:
  - You want MiniMax models in Talon
...
```bash
talon plugins enable minimax-portal-auth  # skip if already loaded.
talon gateway restart  # restart if gateway is already running
talon onboard --auth-choice minimax-portal
```
...
See [MiniMax OAuth plugin README](https://github.com/talon/talon/tree/main/extensions/minimax-portal-auth) for details.
...
Configure manually via `talon.json`:
...
- Use `talon models list` and `talon models set minimax/MiniMax-M2.1` to switch.
...
- Running `talon configure` and selecting **MiniMax M2.1**, or
