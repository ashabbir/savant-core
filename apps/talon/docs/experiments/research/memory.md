...
read_when:
  - Designing workspace memory (~/.talon/workspace) beyond daily Markdown logs
  - Deciding: standalone CLI vs deep Talon integration
...
Target: Clawd-style workspace (`agents.defaults.workspace`, default `~/.talon/workspace`) where “memory” is stored as one Markdown file per day (`memory/YYYY-MM-DD.md`) plus a small set of stable files (e.g. `memory.md`, `SOUL.md`).
...
Keep `~/.talon/workspace` as canonical human-readable memory.
...
```
~/.talon/workspace/.memory/index.sqlite
```
...
Recommendation: **deep integration in Talon**, but keep a separable core library.

### Why integrate into Talon?
...
- You want the agent itself to call the tools:
  - `talon memory recall "…" --k 25 --since 30d`
  - `talon memory reflect --since 7d`
...
Pragmatic take for `~/.talon/workspace`:
