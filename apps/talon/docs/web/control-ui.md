...
- optional prefix: set `gateway.controlUi.basePath` (e.g. `/talon`)
...
If the page fails to load, start the Gateway first: `talon gateway`.
...
you revoke it with `talon devices revoke --device <id> --role <role>`. See
...
- Config: view/edit `~/.talon/talon.json` (`config.get`, `config.set`)
...
(`tailscale-user-login`) when `gateway.auth.allowTailscale` is `true`. Talon
...
Talon **blocks** Control UI connections without device identity.
