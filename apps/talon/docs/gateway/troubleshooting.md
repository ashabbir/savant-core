---
summary: "Quick troubleshooting guide for common Talon failures"
read_when:
...
When Talon misbehaves, here's how to fix it.
...
| ---------------------------------- | ------------------------------------------------------------------------------------------------------ | ------------------------------------------------- |
| `talon status`                  | Local summary: OS + update, gateway reachability/mode, service, agents/sessions, provider config state | First check, quick overview                       |
| `talon status --all`            | Full local diagnosis (read-only, pasteable, safe-ish) incl. log tail                                   | When you need to share a debug report             |
| `talon status --deep`           | Runs gateway health checks (incl. provider probes; requires reachable gateway)                         | When “configured” doesn’t mean “working”          |
| `talon gateway probe`           | Gateway discovery + reachability (local + remote targets)                                              | When you suspect you’re probing the wrong gateway |
| `talon channels status --probe` | Asks the running gateway for channel status (and optionally probes)                                    | When gateway is reachable but channels misbehave  |
| `talon gateway status`          | Supervisor state (launchd/systemd/schtasks), runtime PID/exit, last gateway error                      | When the service “looks loaded” but nothing runs  |
| `talon logs --follow`           | Live logs (best signal for runtime issues)                                                             | When you need the actual failure reason           |

**Sharing output:** prefer `talon status --all` (it redacts tokens). If you paste `talon status`, consider setting `TALON_SHOW_SECRETS=0` first (token previews).
...
- Or paste a setup-token on the **gateway host**:
  ...
  ```bash
  talon models auth setup-token --provider anthropic
  ```
...
```bash
talon models auth paste-token --provider anthropic
talon models status
```
...
- Preferred: `talon logs --follow`
- File logs (always): `/tmp/talon/talon-YYYY-MM-DD.log` (or your configured `logging.file`)
- macOS LaunchAgent (if installed): `$TALON_STATE_DIR/logs/gateway.log` and `gateway.err.log`
- Linux systemd (if installed): `journalctl --user -u talon-gateway[-<profile>].service -n 200 --no-pager`
...
- Or set it directly:
  ...
  ```bash
  talon config set gateway.mode local
  ```
...
- Set a remote URL and keep `gateway.mode=remote`:
  ...
  ```bash
  talon config set gateway.mode remote
  talon config set gateway.remote.url "wss://gateway.example.com"
  ```

**Ad-hoc/dev only:** pass `--allow-unconfigured` to start the gateway without
`gateway.mode=local`.

**No config file yet?** Run `talon setup` to create a starter config, then rerun
the gateway.
...
variables like `DISPLAY` should live in `~/.talon/.env` (loaded early by the
gateway).
...
service was installed with Bun or a version-managed Node path, run `talon doctor`
...
- then run `talon sandbox recreate --agent <id>` (or `--all`)
...
- `gateway.mode` must be `local` for `talon gateway` and the service.
- If you set `gateway.mode=remote`, the **CLI defaults** to a remote URL. The service can still be running locally, but your CLI may be probing the wrong place. Use `talon gateway status` to see the service’s resolved port + probe target (or pass `--url`).
- `talon gateway status` and `talon doctor` surface the **last gateway error** from logs when the service looks running but the port is closed.
- Non-loopback binds (`lan`/`tailnet`/`custom`, or `auto` when loopback is unavailable) require auth:
  `gateway.auth.token` (or `TALON_GATEWAY_TOKEN`).
- `gateway.remote.token` is for remote CLI calls only; it does **not** enable local auth.
- `gateway.token` is ignored; use `gateway.auth.token`.

**If `talon gateway status` shows a config mismatch**
...
- Fix: rerun `talon gateway install --force` from the same `--profile` / `TALON_STATE_DIR` you want the service to use.

**If `talon gateway status` reports service config issues**

- The supervisor config (launchd/systemd/schtasks) is missing current defaults.
- Fix: run `talon doctor` to update it (or `talon gateway install --force` for a full rewrite).
...
- Fix: set `gateway.auth.mode` + `gateway.auth.token` (or export `TALON_GATEWAY_TOKEN`) and restart the service.

**If `talon gateway status` says `bind=tailnet` but no tailnet interface was found**
...
If you upgraded from older installs, you might still have `~/talon` on disk.
...
Symptoms: `pwd` or file tools show `~/.talon/sandboxes/...` even though you
...
Talon intentionally rejects **older/insecure models** (especially those more
...
- If you’re unsure which models are available, run `talon models list` or
  `talon models scan` and choose a supported one.
...
grep -n "agents\\|groupChat\\|mentionPatterns\\|channels\\.whatsapp\\.groups\\|channels\\.telegram\\.groups\\|channels\\.imessage\\.groups\\|channels\\.discord\\.guilds" \
  "${TALON_CONFIG_PATH:-$HOME/.talon/talon.json}"
```
...
tail -f "$(ls -t /tmp/talon/talon-*.log | head -1)" | grep "blocked\\|skip\\|unauthorized"
```
...
```bash
talon logs --follow | grep "pairing request"
```
...
```bash
ls -la ~/.talon/agents/<agentId>/sessions/
```
...
talon logs --limit 200 | grep "connection\\|disconnect\\|logout"
```
...
```bash
talon gateway --verbose
```

If you’re logged out / unlinked:

```bash
talon channels logout
trash "${TALON_STATE_DIR:-$HOME/.talon}/credentials" # if logout can't cleanly remove everything
talon channels login --verbose       # re-scan QR
```
...
```bash
grep "media\\|fetch\\|download" "$(ls -t /tmp/talon/talon-*.log | head -1)" | tail -20
```

### High Memory Usage

Talon keeps conversation history in memory.
...
Talon now refuses to start when the config contains unknown keys, malformed values, or invalid types.
...
- `talon doctor` reports every invalid entry.
- `talon doctor --fix` applies migrations/repairs and rewrites the config.
- Diagnostic commands like `talon logs`, `talon health`, `talon status`, `talon gateway status`, and `talon gateway probe` still run even if the config is invalid.

### “All models failed” — what should I check first?

- **Credentials** present for the provider(s) being tried (auth profiles + env vars).
- **Model routing**: confirm `agents.defaults.model.primary` and fallbacks are models you can access.
- **Gateway logs** in `/tmp/talon/…` for the exact provider error.
- **Model status**: use `/model status` (chat) or `talon models status` (CLI).
...
```bash
curl -fsSL https://talon.ai/install.sh | bash -s -- --install-method git --no-onboard
```
...
5. Run `talon channels status --probe` for audit hints.
...
endpoint accepts a strict subset of JSON Schema. Talon scrubs/normalizes tool
...
If resetting doesn't work, change the `BUNDLE_ID` in [`scripts/package-mac-app.sh`](https://github.com/talon/talon/blob/main/scripts/package-mac-app.sh) (e.g., add a `.test` suffix) and rebuild. This forces macOS to treat it as a new app.
...
talon gateway status
talon gateway stop
# Or: launchctl bootout gui/$UID/bot.molt.gateway (replace with bot.molt.<profile>; legacy com.talon.* still works)
```
...
Ensure the global `talon` CLI is installed and matches the app version:

```bash
talon --version
npm install -g talon@<version>
```
...
#   ${TALON_CONFIG_PATH:-$HOME/.talon/talon.json} -> { logging: { level: "trace" } }
#
# Then run verbose commands to mirror debug output to stdout:
talon gateway --verbose
talon channels login --verbose
```
...
| Log                               | Location                                                                                                                                                                                                                                                                                                                    |
| --------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Gateway file logs (structured)    | `/tmp/talon/talon-YYYY-MM-DD.log` (or `logging.file`)                                                                                                                                                                                                                                                                 |
| Gateway service logs (supervisor) | macOS: `$TALON_STATE_DIR/logs/gateway.log` + `gateway.err.log` (default: `~/.talon/logs/...`; profiles use `~/.talon-<profile>/logs/...`)<br />Linux: `journalctl --user -u talon-gateway[-<profile>].service -n 200 --no-pager`<br />Windows: `schtasks /Query /TN "Talon Gateway (<profile>)" /V /FO LIST` |
| Session files                     | `$TALON_STATE_DIR/agents/<agentId>/sessions/`                                                                                                                                                                                                                                                                            |
| Media cache                       | `$TALON_STATE_DIR/media/`                                                                                                                                                                                                                                                                                                |
| Credentials                       | `$TALON_STATE_DIR/credentials/`                                                                                                                                                                                                                                                                                          |
...
talon gateway status --deep

# Is the gateway reachable?
talon health --json
# If it fails, rerun with connection details:
talon health --verbose
...
trash "${TALON_STATE_DIR:-$HOME/.talon}"
talon channels login         # re-pair WhatsApp
talon gateway restart           # or: talon gateway
```
...
1. Check logs first: `/tmp/talon/` (default: `talon-YYYY-MM-DD.log`, or your configured `logging.file`)
