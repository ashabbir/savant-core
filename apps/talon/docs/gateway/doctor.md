...
`talon doctor` is the repair + migration tool for Talon. It fixes stale
...
- Extra workspace dir detection (`~/talon`).
...
you to run `talon doctor`.

Doctor will:

- Explain which legacy keys were found.
- Show the migration it applied.
- Rewrite `~/.talon/talon.json` with the updated schema.
...
- Sessions store + transcripts:
  - from `~/.talon/sessions/` to `~/.talon/agents/<agentId>/sessions/`
- Agent dir:
  - from `~/.talon/agent/` to `~/.talon/agents/<agentId>/agent/`
- WhatsApp auth state (Baileys):
  - from legacy `~/.talon/credentials/*.json` (except `oauth.json`)
  - to `~/.talon/credentials/whatsapp/<accountId>/...` (default account id: `default`)

These migrations are best-effort and idempotent; doctor will emit warnings when
it leaves any legacy folders behind as backups. The Gateway/CLI also auto-migrates
the legacy sessions + agent dir on startup so history/auth/models land in the
per-agent path without a manual doctor run. WhatsApp auth is intentionally only
migrated via `talon doctor`.
...
- **Multiple state dirs**: warns when multiple `~/.talon` folders exist across
  home directories or when `TALON_STATE_DIR` points elsewhere (history can
  split between installs).
- **Remote mode reminder**: if `gateway.mode=remote`, doctor reminds you to run
  it on the remote host (the state lives there).
- **Config file permissions**: warns if `~/.talon/talon.json` is
...
offers to remove them and install the Talon service using the current gateway
port. It can also scan for extra gateway-like services and print cleanup hints.
Profile-named Talon gateway services are considered first-class and are not
flagged as "extra."
...
generate a token. Use `talon doctor --generate-gateway-token` to force token
...
- `talon doctor` prompts before rewriting supervisor config.
- `talon doctor --yes` accepts the default repair prompts.
- `talon doctor --repair` applies recommended fixes without prompts.
- `talon doctor --repair --force` overwrites custom supervisor configs.
- You can always force a full rewrite via `talon gateway install --force`.
