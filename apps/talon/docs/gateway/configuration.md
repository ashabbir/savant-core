---
summary: "All configuration options for ~/.talon/talon.json with examples"
read_when:
...
Talon reads an optional **JSON5** config from `~/.talon/talon.json` (comments + trailing commas allowed).

If the file is missing, Talon uses safe-ish defaults (embedded Pi agent + per-sender sessions + workspace `~/.talon/workspace`). You usually only need a config to:
...
Talon only accepts configurations that fully match the schema.
Unknown keys, malformed types, or invalid values cause the Gateway to **refuse to start** for safety.

When validation fails:

- The Gateway does not boot.
- Only diagnostic commands are allowed (for example: `talon doctor`, `talon logs`, `talon health`, `talon status`, `talon service`, `talon help`).
- Run `talon doctor` to see the exact issues.
- Run `talon doctor --fix` (or `--yes`) to apply migrations/repairs.
...
use `config.patch` or `talon config set`. Keep a backup of `~/.talon/talon.json`.
...
```bash
talon gateway call config.get --params '{}' # capture payload.hash
talon gateway call config.apply --params '{
...
```bash
talon gateway call config.get --params '{}' # capture payload.hash
talon gateway call config.patch --params '{
...
```json5
{
  agents: {
    ...
    list: [
      {
        id: "main",
        groupChat: { mentionPatterns: ["@talon", "reisponde"] },
      },
    ],
  },
  channels: {
...
// ~/.talon/agents.json5
{
  ...
  list: [{ id: "main", workspace: "~/.talon/workspace" }],
}
```
...
{ "$include": "/etc/talon/base.json5" } // absolute
...
```json5
// ~/.talon/clients/mueller/agents.json5
[
...
```json5
// ~/.talon/clients/mueller/broadcast.json5
{
...
Talon reads env vars from the parent process (shell, launchd/systemd, CI, etc.).
...
Opt-in convenience: if enabled and none of the expected keys are set yet, Talon runs your login shell and imports only the missing expected keys (never overrides).
...
- `TALON_LOAD_SHELL_ENV=1`
- `TALON_SHELL_ENV_TIMEOUT_MS=15000`
...
Talon stores **per-agent** auth profiles (OAuth + API keys) in:

- `<agentDir>/auth-profiles.json` (default: `~/.talon/agents/<agentId>/agent/auth-profiles.json`)

See also: [/concepts/oauth](/concepts/oauth)

Legacy OAuth imports:

- `~/.talon/credentials/oauth.json` (or `$TALON_STATE_DIR/credentials/oauth.json`)
...
- `~/.talon/agent/*` (migrated by `talon doctor` into `~/.talon/agents/<defaultAgentId>/agent/*`)

Overrides:

- OAuth dir (legacy import only): `TALON_OAUTH_DIR`
- Agent dir (default agent root override): `TALON_AGENT_DIR` (preferred), `PI_CODING_AGENT_DIR` (legacy)

On first use, Talon imports `oauth.json` entries into `auth-profiles.json`.
...
If set, Talon derives defaults (only when you haven’t set them explicitly):
...
- Default log file: `/tmp/talon/talon-YYYY-MM-DD.log`
- If you want a stable path, set `logging.file` to `/tmp/talon/talon.log`.
  ...
  channels: {
    whatsapp: {
      accounts: {
        ...
        personal: {},
        biz: {
          // Optional override. Default: ~/.talon/credentials/whatsapp/biz
          // authDir: "~/.talon/credentials/whatsapp/biz",
        },
      },
    },
  },
...
- The legacy single-account Baileys auth dir is migrated by `talon doctor` into `whatsapp/default`.
...
```json5
{
  ...
  agents: {
    list: [{ id: "main", groupChat: { mentionPatterns: ["@talon", "talon"] } }],
  },
}
```
    ...
    list: [
      {
        ...
        groupChat: {
          ...
          mentionPatterns: ["reisponde", "@talon"],
        },
      },
    ],
  },
...
- `agents.list[]`: per-agent overrides.
  ...
  - `workspace`: default `~/.talon/workspace-<agentId>` (for `main`, falls back to `agents.defaults.workspace`).
  - `agentDir`: default `~/.talon/agents/<agentId>/agent`.
...
```json5
{
  agents: {
    list: [
      {
        id: "personal",
        workspace: "~/.talon/workspace-personal",
        sandbox: { mode: "off" },
      },
    ],
  },
...
```json5
{
  agents: {
    list: [
      {
        id: "family",
        workspace: "~/.talon/workspace-family",
        sandbox: {
          mode: "all",
          scope: "agent",
          workspaceAccess: "ro",
...
```json5
{
  agents: {
    list: [
      {
        id: "public",
        workspace: "~/.talon/workspace-public",
        sandbox: {
          mode: "all",
          scope: "agent",
...
```json5
{
  agents: {
    list: [
      { id: "home", default: true, workspace: "~/.talon/workspace-home" },
      { id: "work", workspace: "~/.talon/workspace-work" },
    ],
  },
  bindings: [
...
- `commands.config: true` enables `/config` (reads/writes `talon.json`).
...
Talon starts Telegram only when a `channels.telegram` config section exists. The bot token is resolved from `channels.telegram.botToken` (or `channels.telegram.tokenFile`), with `TELEGRAM_BOT_TOKEN` as a fallback for the default account.
  ...
  channels: {
    discord: {
      ...
      dm: {
        ...
        groupChannels: ["talon-dm"], // optional group DM allowlist
      },
      guilds: {
        "123456789012345678": {
          // guild id (preferred) or slug
          slug: "friends-of-talon",
...
Talon starts Slack when the provider is enabled and both tokens are set (via config or `SLACK_BOT_TOKEN` + `SLACK_APP_TOKEN`). Use `user:<id>` (DM) or `channel:<id>` when specifying delivery targets for cron/CLI commands.
...
Install it first: `talon plugins install @talon/mattermost` (or `./extensions/mattermost` from a git checkout).
...
Talon starts Mattermost when the account is configured (bot token + base URL) and enabled. The token + base URL are resolved from `channels.mattermost.botToken` + `channels.mattermost.baseUrl` or `MATTERMOST_BOT_TOKEN` + `MATTERMOST_URL` for the default account (unless `channels.mattermost.enabled` is `false`).
...
Talon spawns `imsg rpc` (JSON-RPC over stdio). No daemon or port required.
...
Optional repository root to show in the system prompt’s Runtime line. If unset, Talon
...
```json5
{
  agents: { defaults: { repoRoot: "~/Projects/talon" } },
}
```
...
When a file exceeds this limit, Talon logs a warning and injects a truncated
...
message envelopes). If unset, Talon uses the host timezone at runtime.
...
`[talon]`, so same-phone conversations stay legible.
...
`messages.messagePrefix`). Default stays **unchanged**: `"[talon]"` when
`channels.whatsapp.allowFrom` is empty, otherwise `""` (no prefix). When using
`"[talon]"`, Talon will instead use `[{identity.name}]` when the routed
...
Enable text-to-speech for outbound replies. When on, Talon generates audio
...
```json5
{
  messages: {
    tts: {
      ...
      maxTextLength: 4000,
      timeoutMs: 30000,
      prefsPath: "~/.talon/settings/tts.json",
      elevenlabs: {
...
Talon also ships a few built-in alias shorthands. Defaults only apply when the model
...
If you omit the provider, Talon currently assumes `anthropic` as a temporary
...
- `tools.web.search.apiKey` (recommended: set via `talon configure --section web`, or use `BRAVE_API_KEY` env var)
...
- `group:nodes`: `nodes`
- `group:talon`: all built-in Talon tools (excludes provider plugins)
...
- agent workspace access: `workspaceAccess: "none"` (default)
  - `"none"`: use a per-scope sandbox workspace under `~/.talon/sandboxes`
  ...
  agents: {
    defaults: {
      sandbox: {
        ...
        workspaceRoot: "~/.talon/sandboxes",
        docker: {
          image: "talon-sandbox:bookworm-slim",
          containerPrefix: "talon-sbx-",
          workdir: "/workspace",
          readOnlyRoot: true,
        ...
        browser: {
          enabled: false,
          image: "talon-sandbox-browser:bookworm-slim",
          containerPrefix: "talon-sbx-browser-",
          cdpPort: 9222,
          vncPort: 5900,
          noVncPort: 6080,
          headless: false,
...
Talon uses the **pi-coding-agent** model catalog. You can add custom providers
(LiteLLM, local OpenAI-compatible servers, Anthropic proxies, etc.) by writing
`~/.talon/agents/<agentId>/agent/models.json` or by defining the same schema inside your
Talon config under `models.providers`.
Provider-by-provider overview + examples: [/concepts/model-providers](/concepts/model-providers).

When `models.providers` is present, Talon writes/merges a `models.json` into
`~/.talon/agents/<agentId>/agent/` on startup:
...
OpenCode Zen is a multi-model gateway with per-model endpoints. Talon uses
...
- Shortcut: `talon onboard --auth-choice opencode-zen`.
...
Shortcut: `talon onboard --auth-choice zai-api-key`.
...
- Set `MOONSHOT_API_KEY` in the environment or use `talon onboard --auth-choice moonshot-api-key`.
- Model ref: `moonshot/kimi-k2.5`.
- For the China endpoint, either:
  - Run `talon onboard --auth-choice moonshot-api-key-cn` (wizard will set `https://api.moonshot.cn/v1`), or
...
- Set `KIMI_API_KEY` in the environment or use `talon onboard --auth-choice kimi-code-api-key`.
...
- Set `SYNTHETIC_API_KEY` or use `talon onboard --auth-choice synthetic-api-key`.
...
- Set `MINIMAX_API_KEY` environment variable or use `talon onboard --auth-choice minimax-api`.
...
- Override the agent config root with `TALON_AGENT_DIR` (or `PI_CODING_AGENT_DIR`)
  if you want `models.json` stored elsewhere (default: `~/.talon/agents/main/agent`).
...
```json5
{
  session: {
    ...
    // Default is already per-agent under ~/.talon/agents/<agentId>/sessions/sessions.json
    // You can override with {agentId} templating:
    store: "~/.talon/agents/{agentId}/sessions/sessions.json",
...
- `resetByType`: per-session overrides for `dm`, `group`, and `thread`.
  - If you only set legacy `session.idleMinutes` without any `reset`/`resetByType`, Talon stays in idle-only mode for backward compatibility.
...
overrides. Applies to **bundled** skills and `~/.talon/skills` (workspace skills
...
from `~/.talon/extensions`, `<workspace>/.talon/extensions`, plus any
...
### `browser` (talon-managed browser)

Talon can start a **dedicated, isolated** Chrome/Brave/Edge/Chromium instance for talon and expose a small loopback control service.
...
- Note: the control server is started by the running gateway (Talon.app menubar, or `talon gateway`).
...
```json5
{
  browser: {
    ...
    profiles: {
      talon: { cdpPort: 18800, color: "#FF4500" },
...
```json5
{
  gateway: {
    ...
    bind: "loopback",
    // controlUi: { enabled: true, basePath: "/talon" }
...
- When a connection comes from one of these IPs, Talon uses `x-forwarded-for` (or `x-real-ip`) to determine the client IP for local pairing checks and HTTP auth/local checks.
- Only list proxies you fully control, and ensure they **overwrite** incoming `x-forwarded-for`.

Notes:

- `talon gateway` refuses to start unless `gateway.mode` is set to `local` (or you pass the override flag).
- `gateway.port` controls the single multiplexed port used for WebSocket + HTTP (control UI, hooks, A2UI).
- OpenAI Chat Completions endpoint: **disabled by default**; enable with `gateway.http.endpoints.chatCompletions.enabled: true`.
- Precedence: `--port` > `TALON_GATEWAY_PORT` > `gateway.port` > default `18789`.
...
- `gateway.auth.password` can be set here, or via `TALON_GATEWAY_PASSWORD` (recommended).
- `gateway.auth.allowTailscale` allows Tailscale Serve identity headers
  (`tailscale-user-login`) to satisfy auth when the request arrives on loopback
  with `x-forwarded-for`, `x-forwarded-proto`, and `x-forwarded-host`. Talon
...
- Talon.app watches `~/.talon/talon.json` and switches modes live when `gateway.mode` or `gateway.remote.url` changes.
...
The Gateway watches `~/.talon/talon.json` (or `TALON_CONFIG_PATH`) and applies changes automatically.
...
- `~/.talon/talon.json` (or `TALON_CONFIG_PATH`)
...
- `TALON_STATE_DIR` (sessions/creds)
- `agents.defaults.workspace` (memories)
- `gateway.port` (unique per instance)

Convenience flags (CLI):

- `talon --dev …` → uses `~/.talon-dev` + shifts ports from base `19001`
- `talon --profile <name> …` → uses `~/.talon-<name>` (port via config/env/flags)
...
```bash
TALON_CONFIG_PATH=~/.talon/a.json \
TALON_STATE_DIR=~/.talon-a \
talon gateway --port 19001
```
...
Gmail helper config (used by `talon webhooks gmail setup` / `run`):

```json5
{
  hooks: {
    gmail: {
      account: "talon@gmail.com",
...
- Set `TALON_SKIP_GMAIL_WATCHER=1` to disable the auto-start (for manual runs).
...
Note: when `tailscale.mode` is on, Talon defaults `serve.path` to `/` so
...
Default root: `~/.talon/workspace/canvas`  
Default port: `18793` (chosen to avoid the talon browser CDP port `18792`)  
...
- watches the directory and broadcasts reloads over a WebSocket endpoint at `/__talon__/ws`
- auto-creates a starter `index.html` when the directory is empty (so you see something immediately)
- also serves A2UI at `/__talon__/a2ui/` and is advertised to nodes as `canvasHostUrl`
...
```json5
{
  bridge: {
    ...
    tls: {
      enabled: true,
      // Uses ~/.talon/bridge/tls/bridge-{cert,key}.pem when omitted.
      // certPath: "~/.talon/bridge/tls/bridge-cert.pem",
      // keyPath: "~/.talon/bridge/tls/bridge-key.pem"
    },
  },
}
```
...
Controls LAN mDNS discovery broadcasts (`_talon-gw._tcp`).

- `minimal` (default): omit `cliPath` + `sshPort` from TXT records
- `full`: include `cliPath` + `sshPort` in TXT records
- `off`: disable mDNS broadcasts entirely
- Hostname: defaults to `talon` (advertises `talon.local`). Override with `TALON_MDNS_HOSTNAME`.
...
When enabled, the Gateway writes a unicast DNS-SD zone for `_talon-gw._tcp` under `~/.talon/dns/` using the configured discovery domain (example: `talon.internal.`).
