...
- During heartbeat runs, Talon treats `HEARTBEAT_OK` as an ack when it appears
...
- `session`: optional session key for heartbeat runs.
  ...
  - Explicit session key (copy from `talon sessions --json` or the [sessions CLI](/cli/sessions)).
...
If **all three** are false, Talon skips the heartbeat run entirely (no model call).
...
```bash
talon system event --text "Check for urgent follow-ups" --mode now
```
