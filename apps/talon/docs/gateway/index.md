...
- Replaces the legacy `gateway` command. CLI entry point: `talon gateway`.
- Runs until stopped; exits non-zero on fatal errors so the supervisor restarts it.

## How to run (local)

```bash
talon gateway --port 18789
# for full debug/trace logs in stdio:
talon gateway --port 18789 --verbose
...
- Config hot reload watches `~/.talon/talon.json` (or `TALON_CONFIG_PATH`).
...
- Starts a Canvas file server by default on `canvasHost.port` (default `18793`), serving `http://<gateway-host>:18793/__talon__/canvas/` from `~/.talon/workspace/canvas`. Disable with `canvasHost.enabled=false` or `TALON_SKIP_CANVAS_HOST=1`.
...
- Gateway auth is required by default: set `gateway.auth.token` (or `TALON_GATEWAY_TOKEN`) or `gateway.auth.password`. Clients must send `connect.params.auth.token/password` unless using Tailscale Serve identity.
- The wizard now generates a token by default, even on loopback.
- Port precedence: `--port` > `TALON_GATEWAY_PORT` > `gateway.port` > default `18789`.
...
- macOS: `bot.molt.<profile>` (legacy `com.talon.*` may still exist)
- Linux: `talon-gateway-<profile>.service`
- Windows: `Talon Gateway (<profile>)`

Install metadata is embedded in the service config:

- `TALON_SERVICE_MARKER=talon`
- `TALON_SERVICE_KIND=gateway`
- `TALON_SERVICE_VERSION=<version>`
...
```bash
talon --dev setup
talon --dev gateway --allow-unconfigured
...
- `TALON_STATE_DIR=~/.talon-dev`
- `TALON_CONFIG_PATH=~/.talon-dev/talon.json`
- `TALON_GATEWAY_PORT=19001` (Gateway WS + HTTP)
- browser control service port = `19003` (derived: `gateway.port+2`, loopback only)
- `canvasHost.port=19005` (derived: `gateway.port+4`)
- `agents.defaults.workspace` default becomes `~/.talon/workspace-dev` when you run `setup`/`onboard` under `--dev`.

Derived ports (rules of thumb):

- Base port = `gateway.port` (or `TALON_GATEWAY_PORT` / `--port`)
- browser control service port = base + 2 (loopback only)
- `canvasHost.port = base + 4` (or `TALON_CANVAS_HOST_PORT` / config override)
- Browser profile CDP ports auto-allocate from `browser.controlPort + 9 .. + 108` (persisted per profile).

Checklist per instance:

- unique `gateway.port`
- unique `TALON_CONFIG_PATH`
- unique `TALON_STATE_DIR`
...
```bash
talon --profile main gateway install
talon --profile rescue gateway install
```

Example:

```bash
TALON_CONFIG_PATH=~/.talon/a.json TALON_STATE_DIR=~/.talon-a talon gateway --port 19001
TALON_CONFIG_PATH=~/.talon/b.json TALON_STATE_DIR=~/.talon-b talon gateway --port 19002
```
...
- `health` — full health snapshot (same shape as `talon health --json`).
...
- Use launchd to keep the service alive:
  - Program: path to `talon`
  - Arguments: `gateway`
  - KeepAlive: true
  - StandardOut/Err: file paths or `syslog`
- On failure, launchd restarts; fatal misconfig should keep exiting so the operator notices.
- LaunchAgents are per-user and require a logged-in session; for headless setups use a custom LaunchDaemon (not shipped).
  - `talon gateway install` writes `~/Library/LaunchAgents/bot.molt.gateway.plist`
    (or `bot.molt.<profile>.plist`; legacy `com.talon.*` is cleaned up).
  - `talon doctor` audits the LaunchAgent config and can update it to current defaults.
...
- If other gateway-like services are detected, the CLI warns unless they are Talon profile services.
  ...
  - Cleanup: `talon gateway uninstall` (current service) and `talon doctor` (legacy migrations).
- `gateway install` is a no-op when already installed; use `talon gateway install --force` to reinstall (profile/env/path changes).

Bundled mac app:

- Talon.app can bundle a Node-based gateway relay and install a per-user LaunchAgent labeled
  `bot.molt.gateway` (or `bot.molt.<profile>`; legacy `com.talon.*` labels still unload cleanly).
- To stop it cleanly, use `talon gateway stop` (or `launchctl bootout gui/$UID/bot.molt.gateway`).
- To restart, use `talon gateway restart` (or `launchctl kickstart -k gui/$UID/bot.molt.gateway`).
  - `launchctl` only works if the LaunchAgent is installed; otherwise use `talon gateway install` first.
  - Replace the label with `bot.molt.<profile>` when running a named profile.

## Supervision (systemd user unit)

Talon installs a **systemd user service** by default on Linux/WSL2. We
...
`talon gateway install` writes the user unit. `talon doctor` audits the
unit and can update it to match the current recommended defaults.

Create `~/.config/systemd/user/talon-gateway[-<profile>].service`:

```
[Unit]
Description=Talon Gateway (profile: <profile>, v<version>)
After=network-online.target
Wants=network-online.target

[Service]
ExecStart=/usr/local/bin/talon gateway --port 18789
Restart=always
RestartSec=5
Environment=TALON_GATEWAY_TOKEN=
...
```
systemctl --user enable --now talon-gateway[-<profile>].service
```

**Alternative (system service)** - for always-on or multi-user servers, you can
install a systemd **system** unit instead of a user unit (no lingering needed).
Create `/etc/systemd/system/talon-gateway[-<profile>].service` (copy the unit above,
switch `WantedBy=multi-user.target`, set `User=` + `WorkingDirectory=`), then:

```
sudo systemctl daemon-reload
sudo systemctl enable --now talon-gateway[-<profile>].service
```
...
- `talon gateway health|status` — request health/status over the Gateway WS.
- `talon message send --target <num> --message "hi" [--media ...]` — send via Gateway (idempotent for WhatsApp).
- `talon agent --message "hi" --to <num>` — run an agent turn (waits for final by default).
- `talon gateway call <method> --params '{"k":"v"}'` — raw method invoker for debugging.
- `talon gateway stop|restart` — stop/restart the supervised gateway service (launchd/systemd).
- Gateway helper subcommands assume a running gateway on `--url`; they no longer auto-spawn one.

## Migration guidance

- Retire uses of `talon gateway` and the legacy TCP control port.
