...
Talon integrates external CLIs via JSON-RPC. Two patterns are used today.
...
- Talon owns lifecycle when `channels.signal.autoStart=true`.
...
- Talon spawns `imsg rpc` as a child process (legacy iMessage integration).
