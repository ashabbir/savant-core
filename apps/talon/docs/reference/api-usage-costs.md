...
Talon features that can generate provider usage or paid API calls.
...
- `talon status --usage` and `talon channels list` show provider **usage windows**
...
Talon can pick up credentials from:
...
`talon models scan` can probe OpenRouter models and uses `OPENROUTER_API_KEY` when
probing is enabled.
