...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in Talon
...
Running Talon via Node with `tsx` fails at startup with:

```
[talon] Failed to start CLI: TypeError: __name is not a function
...
- Before that (Bun path), `talon status` and `gateway:watch` worked.
...
- Use Node + tsc watch, then run compiled output:
  ...
  node --watch talon.mjs status
  ```

- Confirmed locally: `pnpm exec tsc -p tsconfig.json` + `node talon.mjs status` works on Node 25.
