...
This guide summarizes a sane workflow for working on the pi integration in Talon.
...
- Trigger the agent directly:
  - `pnpm talon agent --message "Hello" --thinking low`
...
State lives under the Talon state directory. Default is `~/.talon`. If `TALON_STATE_DIR` is set, use that directory instead.

To reset everything:

- `talon.json` for config
