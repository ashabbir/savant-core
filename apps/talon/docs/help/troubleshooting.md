...
You can also set `TALON_VERBOSE=1` instead of the flag.
...
### `docs.talon.ai` shows an SSL error (Comcast/Xfinity)

Some Comcast/Xfinity connections block `docs.talon.ai` via Xfinity Advanced Security.
Disable Advanced Security or add `docs.talon.ai` to the allowlist, then retry.
...
- Check the allowlist: `talon config get agents.defaults.models`
...
If you can, include the relevant log tail from `talon logs --follow`.
