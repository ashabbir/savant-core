...
- `/usage cost` in chats: local cost summary aggregated from Talon session logs.
- CLI: `talon status --usage` prints a full per-provider breakdown.
- CLI: `talon channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).
