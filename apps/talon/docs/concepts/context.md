...
read_when:
  - You want to understand what “context” means in Talon
...
“Context” is **everything Talon sends to the model for a run**. It is bounded by the model’s **context window** (token limit).

Beginner mental model:

- **System prompt** (Talon-built): rules, tools, skills list, time/runtime, and injected workspace files.
...
## How Talon builds the system prompt

The system prompt is **Talon-owned** and rebuilt each run. It includes:
...
By default, Talon injects a fixed set of workspace files (if present):
