...
你可以在 `~/.talon/talon.json` 中覆盖此设置：

```json
{
  "logging": {
    "file": "/path/to/talon.log"
  }
}
```
...
```bash
talon channels logs --channel whatsapp
```
...
所有日志配置都在 `~/.talon/talon.json` 的 `logging` 下。

```json
{
  "logging": {
    "level": "info",
    "file": "/tmp/talon/talon-YYYY-MM-DD.log",
    "consoleLevel": "info",
    "consoleStyle": "pretty",
...
- Talon 目前通过 **OTLP/HTTP（protobuf）** 导出。
...
```
TALON_DIAGNOSTICS=telegram.http,telegram.payload
```
...
- 你也可以使用 `talon plugins enable diagnostics-otel` 启用插件。
...
- `talon.tokens`（计数器，属性：`talon.token`、`talon.channel`、`talon.provider`、`talon.model`）
- `talon.cost.usd`（计数器，属性：`talon.channel`、`talon.provider`、`talon.model`）
- `talon.run.duration_ms`（直方图，属性：`talon.channel`、`talon.provider`、`talon.model`）
- `talon.context.tokens`（直方图，属性：`talon.context`、`talon.channel`、`talon.provider`、`talon.model`）

消息流：

- `talon.webhook.received`（计数器，属性：`talon.channel`、`talon.webhook`）
- `talon.webhook.error`（计数器，属性：`talon.channel`、`talon.webhook`）
- `talon.webhook.duration_ms`（直方图，属性：`talon.channel`、`talon.webhook`）
- `talon.message.queued`（计数器，属性：`talon.channel`、`talon.source`）
- `talon.message.processed`（计数器，属性：`talon.channel`、`talon.outcome`）
- `talon.message.duration_ms`（直方图，属性：`talon.channel`、`talon.outcome`）

队列 + 会话：

- `talon.queue.lane.enqueue`（计数器，属性：`talon.lane`）
- `talon.queue.lane.dequeue`（计数器，属性：`talon.lane`）
- `talon.queue.depth`（直方图，属性：`talon.lane` 或 `talon.channel=heartbeat`）
- `talon.queue.wait_ms`（直方图，属性：`talon.lane`）
- `talon.session.state`（计数器，属性：`talon.state`、`talon.reason`）
- `talon.session.stuck`（计数器，属性：`talon.state`）
- `talon.session.stuck_age_ms`（直方图，属性：`talon.state`）
- `talon.run.attempt`（计数器，属性：`talon.attempt`）

### 导出的 span（名称 + 关键属性）

- `talon.model.usage`
  - `talon.channel`、`talon.provider`、`talon.model`
  - `talon.sessionKey`、`talon.sessionId`
  - `talon.tokens.*`（input/output/cache_read/cache_write/total）
- `talon.webhook.processed`
  - `talon.channel`、`talon.webhook`、`talon.chatId`
- `talon.webhook.error`
  - `talon.channel`、`talon.webhook`、`talon.chatId`、`talon.error`
- `talon.message.processed`
  - `talon.channel`、`talon.outcome`、`talon.chatId`、`talon.messageId`、`talon.sessionKey`、`talon.sessionId`、`talon.reason`
- `talon.session.stuck`
  - `talon.state`、`talon.ageMs`、`talon.queueDepth`、`talon.sessionKey`、`talon.sessionId`
...
- **Gateway 网关无法访问？** 先运行 `talon doctor`。
