...
function restoreEnv(snapshot: EnvSnapshot) {
  ...
  restoreKey("TALON_STATE_DIR", snapshot.stateDir);
}
...
function setTempHome(base: string) {
  ...
  process.env.TALON_STATE_DIR = path.join(base, ".talon");
...
export async function withTempHome<T>(
  ...
  const base = await fs.mkdtemp(path.join(os.tmpdir(), opts.prefix ?? "talon-test-home-"));
  ...
  setTempHome(base);
  await fs.mkdir(path.join(base, ".talon", "agents", "main", "sessions"), { recursive: true });
  if (opts.env) {
