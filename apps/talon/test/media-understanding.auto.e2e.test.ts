...
import type { TalonConfig } from "../src/config/config.js";
...
const makeTempMedia = async (ext: string) => {
  const dir = await makeTempDir("talon-media-e2e-");
...
describe("media understanding auto-detect (e2e)", () => {
  ...
  it("uses sherpa-onnx-offline when available", async () => {
    ...
    try {
      const binDir = await makeTempDir("talon-bin-sherpa-");
      const modelDir = await makeTempDir("talon-sherpa-model-");
      ...
      const cfg: TalonConfig = { tools: { media: { audio: {} } } };
  ...
  it("uses whisper-cli when sherpa is missing", async () => {
    const snapshot = envSnapshot();
    try {
      const binDir = await makeTempDir("talon-bin-whispercpp-");
      const modelDir = await makeTempDir("talon-whispercpp-model-");
      tempPaths.push(binDir, modelDir);
  ...
  it("uses gemini CLI for images when available", async () => {
    const snapshot = envSnapshot();
    try {
      const binDir = await makeTempDir("talon-bin-gemini-");
      ...
      const cfg: TalonConfig = { tools: { media: { image: {} } } };
