...
const spawnGatewayInstance = async (name: string): Promise<GatewayInstance> => {
  ...
  const gatewayToken = `gateway-${name}-token`;
  const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), `talon-e2e-${name}-`));
  const configDir = path.join(homeDir, ".talon");
...
const runCliJson = async (args: string[], env: NodeJS.ProcessEnv): Promise<unknown> => {
  const stdout: string[] = [];
  const stderr: string[] = [];
  const child = spawn("node", ["dist/entry.js", ...args], {
    cwd: process.cwd(),
    env: { ...process.env, ...env },
    stdio: ["ignore", "pipe", "pipe"],
  });
  ...
  try {
    ...
    const jsonStr = startIndex !== -1 ? out.slice(startIndex) : out;
...
const waitForNodeStatus = async (inst: GatewayInstance, nodeId: string, timeoutMs = 10_000) => {
  ...
  while (Date.now() < deadline) {
    const list = (await runCliJson(
      ...
      {
        TALON_GATEWAY_TOKEN: inst.gatewayToken,
        TALON_GATEWAY_PASSWORD: "",
        TALON_DISABLE_CONFIG_CACHE: "1",
      },
    )) as NodeListPayload;
  ...
  it(
    ...
    async () => {
      ...
      const [hookResA, hookResB] = await Promise.all([
        postJson(`http://127.0.0.1:${gwA.port}/hooks/wake`, {
          text: "wake a",
          mode: "now",
        }, { "X-Talon-Token": gwA.hookToken }),
        postJson(`http://127.0.0.1:${gwB.port}/hooks/wake`, {
          text: "wake b",
          mode: "now",
        }, { "X-Talon-Token": gwB.hookToken }),
      ]);
