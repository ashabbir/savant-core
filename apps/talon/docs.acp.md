...
This document describes how the Talon ACP (Agent Client Protocol) bridge works,
how it maps ACP sessions to Gateway sessions, and how IDEs should invoke it.

## Overview

`talon acp` exposes an ACP agent over stdio and forwards prompts to a running
Talon Gateway over WebSocket. It keeps ACP session ids mapped to Gateway
...
drive a Talon Gateway session.

Quick steps:

1. Run a Gateway (local or remote).
2. Configure the Gateway target (`gateway.remote.url` + auth) or pass flags.
3. Point the IDE to run `talon acp` over stdio.

Example config:

```bash
talon config set gateway.remote.url wss://gateway-host:18789
talon config set gateway.remote.token <token>
```

Example run:

```bash
talon acp --url wss://gateway-host:18789 --token <token>
```
...
```bash
talon acp --session agent:main:main
talon acp --session agent:design:main
talon acp --session agent:qa:bug-123
```
...
In Zed, open the Agent panel and select “Talon ACP” to start a thread.

## Execution Model

- ACP client spawns `talon acp` and speaks ACP messages over stdio.
...
```bash
talon acp --session agent:main:main
talon acp --session-label "support inbox"
talon acp --reset-session
```
...
`talon acp` resolves the Gateway URL and auth from CLI flags or config:
