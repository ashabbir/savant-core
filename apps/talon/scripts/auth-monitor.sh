...
#
# Suggested cron: */30 * * * * /home/admin/talon/scripts/auth-monitor.sh
#
# Environment variables:
#   NOTIFY_PHONE - Phone number to send Talon notification (e.g., +1234567890)
#   NOTIFY_NTFY  - ntfy.sh topic for push notifications (e.g., talon-alerts)
...
STATE_FILE="$HOME/.talon/auth-monitor-state"
...
send_notification() {
    ...
    # Send via Talon if phone configured and auth still valid
    if [ -n "$NOTIFY_PHONE" ]; then
        # Check if we can still use talon
        if "$SCRIPT_DIR/claude-auth-status.sh" simple 2>/dev/null | grep -q "OK\|EXPIRING"; then
            echo "Sending via Talon to $NOTIFY_PHONE..."
            talon send --to "$NOTIFY_PHONE" --message "$message" 2>/dev/null || true
        fi
    fi
...
if [ "$DIFF_MS" -lt 0 ]; then
    send_notification "Claude Code auth EXPIRED! Talon is down. Run: ssh l36 '~/talon/scripts/mobile-reauth.sh'" "urgent"
    exit 1
