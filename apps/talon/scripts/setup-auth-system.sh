#!/bin/bash
# Setup Talon Auth Management System
...
echo "  2. Subscribe to a topic (e.g., 'talon-alerts')"
echo ""
...
# Phone notification
echo ""
echo "  Talon message: Send warning via Talon itself"
echo "Enter your phone number for alerts (or leave blank to skip):"
read -r PHONE_NUMBER

# Update service file
SERVICE_FILE="$SCRIPT_DIR/systemd/talon-auth-monitor.service"
...
cp "$SCRIPT_DIR/systemd/talon-auth-monitor.service" ~/.config/systemd/user/
cp "$SCRIPT_DIR/systemd/talon-auth-monitor.timer" ~/.config/systemd/user/
systemctl --user daemon-reload
systemctl --user enable --now talon-auth-monitor.timer
...
echo "  - Auth monitor: systemctl --user status talon-auth-monitor.timer"
echo ""
echo "Quick commands:"
