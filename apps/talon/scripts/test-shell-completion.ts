 ...
 * `talon update`. Use it to verify the completion installation flow
...
\`talon update\`, \`talon doctor\`, and \`talon onboard\`.
...
async function main() {
  ...
  if (status.profileInstalled && status.cacheExists && !options.force) {
    ...
    console.log(
      theme.muted("  1. Remove the '# Talon Completion' block from your shell profile"),
    );
  ...
  if (isCancel(shouldInstall) || !shouldInstall) {
    console.log(theme.muted(`Skipped. Run \`talon completion --install\` later to enable.`));
    return;
  }
