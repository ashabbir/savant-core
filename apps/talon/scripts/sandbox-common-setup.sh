...
BASE_IMAGE="${BASE_IMAGE:-talon-sandbox:bookworm-slim}"
TARGET_IMAGE="${TARGET_IMAGE:-talon-sandbox-common:bookworm-slim}"
...
If you want a clean re-create, remove old sandbox containers:
  docker rm -f \$(docker ps -aq --filter label=talon.sandbox=1)
NOTE
