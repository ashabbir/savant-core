...
# Build and bundle Talon into a minimal .app we can open.
# Outputs to dist/Talon.app

ROOT_DIR="$(cd "$(dirname "$0")/.." && pwd)"
APP_ROOT="$ROOT_DIR/dist/Talon.app"
BUILD_ROOT="$ROOT_DIR/apps/macos/.build"
PRODUCT="Talon"
BUNDLE_ID="${BUNDLE_ID:-ai.talon.mac.debug}"
...
SPARKLE_FEED_URL="${SPARKLE_FEED_URL:-https://raw.githubusercontent.com/talon/talon/main/appcast.xml}"
AUTO_CHECKS=true
...
INFO_PLIST_SRC="$ROOT_DIR/apps/macos/Sources/Talon/Resources/Info.plist"
...
/usr/libexec/PlistBuddy -c "Set :TalonBuildTimestamp ${BUILD_TS}" "$APP_ROOT/Contents/Info.plist" || true
/usr/libexec/PlistBuddy -c "Set :TalonGitCommit ${GIT_COMMIT}" "$APP_ROOT/Contents/Info.plist" || true
...
echo "🚚 Copying binary"
cp "$BIN_PRIMARY" "$APP_ROOT/Contents/MacOS/Talon"
if [[ "${#BUILD_ARCHS[@]}" -gt 1 ]]; then
  BIN_INPUTS=()
  for arch in "${BUILD_ARCHS[@]}"; do
    BIN_INPUTS+=("$(bin_for_arch "$arch")")
  done
  /usr/bin/lipo -create "${BIN_INPUTS[@]}" -output "$APP_ROOT/Contents/MacOS/Talon"
fi
chmod +x "$APP_ROOT/Contents/MacOS/Talon"
# SwiftPM outputs ad-hoc signed binaries; strip the signature before install_name_tool to avoid warnings.
/usr/bin/codesign --remove-signature "$APP_ROOT/Contents/MacOS/Talon" 2>/dev/null || true
...
cp "$ROOT_DIR/apps/macos/Sources/Talon/Resources/Talon.icns" "$APP_ROOT/Contents/Resources/Talon.icns"

echo "📦 Copying device model resources"
rm -rf "$APP_ROOT/Contents/Resources/DeviceModels"
cp -R "$ROOT_DIR/apps/macos/Sources/Talon/Resources/DeviceModels" "$APP_ROOT/Contents/Resources/DeviceModels"
...
echo "📦 Copying TalonKit resources"
TALONKIT_BUNDLE="$(build_path_for_arch "$PRIMARY_ARCH")/$BUILD_CONFIG/TalonKit_TalonKit.bundle"
if [ -d "$TALONKIT_BUNDLE" ]; then
  rm -rf "$APP_ROOT/Contents/Resources/TalonKit_TalonKit.bundle"
  cp -R "$TALONKIT_BUNDLE" "$APP_ROOT/Contents/Resources/TalonKit_TalonKit.bundle"
else
  echo "WARN: TalonKit resource bundle not found at $TALONKIT_BUNDLE (continuing)" >&2
fi
...
echo "⏹  Stopping any running Talon"
killall -q Talon 2>/dev/null || true
