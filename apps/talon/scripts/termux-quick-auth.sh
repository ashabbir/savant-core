...
STATUS=$(ssh -o ConnectTimeout=5 "$SERVER" '$HOME/talon/scripts/claude-auth-status.sh simple' 2>&1)
...
case "$STATUS" in
    ...
    *EXPIRED*|*MISSING*)
        ...
        sleep 2
        termux-notification -t "Talon Re-Auth" -c "After getting key, run: ssh $SERVER '~/talon/scripts/mobile-reauth.sh'" --id talon-auth
        ;;
    *)
