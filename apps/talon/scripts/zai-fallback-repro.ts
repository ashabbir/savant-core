...
async function main() {
  ...
  const baseDir = await fs.mkdtemp(path.join(os.tmpdir(), "talon-zai-fallback-"));
  const stateDir = path.join(baseDir, "state");
  const configPath = path.join(baseDir, "talon.json");
  ...
  const sessionId =
    process.env.TALON_ZAI_FALLBACK_SESSION_ID ??
    process.env.TALON_ZAI_FALLBACK_SESSION_ID ??
    randomUUID();
  ...
  const run1 = await runCommand(
    "run1",
    ["talon", "agent", "--local", "--session-id", sessionId, "--message", toolPrompt],
    envValidAnthropic,
  );
  if (run1.code !== 0) {
  ...
  const run2 = await runCommand(
    "run2",
    ["talon", "agent", "--local", "--session-id", sessionId, "--message", followupPrompt],
    envInvalidAnthropic,
  );
