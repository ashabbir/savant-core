...
ZIP=${1:?"Usage: $0 Talon-<ver>.zip"}
FEED_URL=${2:-"https://raw.githubusercontent.com/talon/talon/main/appcast.xml"}
...
if [[ -z "$VERSION" ]]; then
  if [[ "$ZIP_NAME" =~ ^Talon-([0-9]+(\.[0-9]+){1,2}([-.][^.]*)?)\.zip$ ]]; then
...
DOWNLOAD_URL_PREFIX=${SPARKLE_DOWNLOAD_URL_PREFIX:-"https://github.com/talon/talon/releases/download/v${VERSION}/"}
