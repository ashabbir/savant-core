...
# Check auth status
termux-toast "Checking Talon auth..."

STATUS=$(ssh "$SERVER" '$HOME/talon/scripts/claude-auth-status.sh simple' 2>&1)
EXIT_CODE=$?

case "$STATUS" in
    OK)
        # Get remaining time
        DETAILS=$(ssh "$SERVER" '$HOME/talon/scripts/claude-auth-status.sh json' 2>&1)
    ...
    CLAUDE_EXPIRING|TALON_EXPIRING|TALON_EXPIRING)
...
3. SSH to server and paste key"
                ...
                termux-toast "Run: ssh $SERVER '$HOME/talon/scripts/mobile-reauth.sh'"
                ;;
            *)
    ...
    CLAUDE_EXPIRED|TALON_EXPIRED|TALON_EXPIRED)
...
2. Return here and tap OK to SSH"
                ...
                ;;
            *)
                termux-toast "Warning: Talon won't work until re-auth"
                ;;
        esac
        ;;
